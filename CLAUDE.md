...
- **イベントカードの「本当の公開日」表示（設計書207、2026-09-15実装。同日中にユーザー追加依頼でエリア表示を削除）**: くらし側ニュースカードには元々あった「ソース+本当の公開日(publishedAt)」表示がおでかけ側イベントカードには無かったため新規収集・表示を追加した。`filter-events.js`の`item`組み立て時、`original.pubDate`（`fetch-events.js`が中間オブジェクトに保持するRSSのpubDate）から`publishedAt`（ISO文字列、パース不能・欠落時は取得時刻にフォールバック）を算出し`item.publishedAt`として保存。`fetched_at`（自システムへの取り込み日時）の既存ロジックとは独立、変更なし。フロント（`public/app.js`の`renderEventCard()`）の`metaRowHtml`表示順は最終的に「カテゴリバッジ→`e.source`（ソース名）→`_formatLifeInfoDate(e.publishedAt || e.fetched_at)`（公開日M/D形式）→`e.period`/`e.hours`（開催期間）→New/残り日数バッジ」（初回実装では`e.location`〈エリア〉も期間の手前に表示していたが、同日中のユーザー追加依頼により表示からのみ削除。`e.location`/`e.area`のデータ収集ロジック自体は無変更で保持されたまま）。**既存イベント（実装時点で126件）には`publishedAt`を遡って付与しておらず、今後新規に取り込まれるイベントにのみ付与される**（表示時は`e.publishedAt || e.fetched_at`のフォールバックにより既存イベントも取り込み日で表示され、表示崩れは発生しない）
