...
- **カテゴリフィルターチップの並び順**: `#calendar-filter-row`は「すべて・SG祝日・主要行事・学校休暇・日本の祝日」の順（2026-09-06「日本の祝日は学校行事の後にして」との要望で日本の祝日を最後に移動）。スワイプでのカテゴリ切り替え(`_switchCalCatBySwipe()`)はDOM順を動的に読むため、チップの並び替えだけでJS変更は不要
