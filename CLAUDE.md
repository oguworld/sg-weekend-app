...
| `scripts/run-fetch-extra.sh` | `fetch-events.js --city=sg` → `check-content-integrity.js --city=sg` → `fetch-life-info.js --city=sg`（19:30 SGT実行分のみユーザー向けプッシュ通知あり、詳細は下記参照）→ `notify-fetch-summary.js`（設計書187で追加。開発者向けLINE通知） | 毎日 12:30 SGT・19:30 SGT |
