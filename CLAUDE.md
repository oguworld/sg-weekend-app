...
- **PSI・デング熱のクライテリア表示（2026-09-07追加）**: `#stat-widget-info`内のPSI・デング熱の`.stat-col`（`data-stat-crit="psi"`/`"dengue"`）をタップすると、`#stat-criteria-popover`にレベル一覧をピル型チップ（`.stat-criteria-chip`）で表示し、現在の値に該当するチップだけ`.active`で強調する。判定基準は`STAT_CRITERIA`（`public/app.js`）に定義（PSI: 良好0-50/普通51-100/要注意101-200/健康に悪い201-300/危険301+、デング熱: 警報なし0/注意1-5/警戒6-15/厳重警戒16+）。**`server.js`の`psiLevel()`/`dengueLevel()`としきい値・ラベル文言を必ず一致させること**（サーバー側のレベル文字列と`STAT_CRITERIA`のラベルを文字列一致でマッチングして強調表示しているため、ズレると強調が効かなくなる）。タップ判定は他のタッチ委譲パターンと同様`onclick`(マウス)＋`#stat-widget-info`への`touchend`委譲(タッチ)の両対応。他の場所をタップすると閉じる（`document`全体の`touchend`リスナーで`[data-stat-crit]`/`#stat-criteria-popover`以外へのタップを検知）
...
- **「すべて」表示時のみ末尾に少しの余白を確保**: 今月合わせスクロールの対象は「すべて」のみになったため、今月が12月に近い場合でも12月分のカードが表示できる程度の余白を、`renderCalendarList()`で`_calendarCategory === ''`の時だけ`#calendar-list`に`paddingBottom: 140px`固定値で付与（他のカテゴリでは`''`にリセット）。当初は`scrollEl.clientHeight`（画面高さ分まるごと）にしていたが、手動で一番下までスクロールした時に空白が大きすぎるとの指摘で控えめな固定値に変更（2026-09-07）
...
- **SG祝日バッジのみ配色パレットに関わらず常に薄い赤固定**: `holiday-sg`だけは`--holiday-red`/`--holiday-red-pale`という専用CSS変数(`public/app.css`の`:root`)を使っており、他のカテゴリ色(`--gold`/`--sky`/`--plum`等)と違って**柳グリーンパレット選択時にも上書きされず常に赤のまま**になるよう意図的に設計している（2026-09-06「祝日のラベルはさすがに薄い赤にしよう」という要望より。日本のカレンダー文化で祝日=赤という直感に合わせるため、テーマの統一感より視認性を優先した例外）。**`holiday-jp`(日本の祝日)は対象外**で、従来通り`--sky`/`--sky-pale`(青系、テーマに応じて変化)のまま（2026-09-06「赤にするのはSGの祝日だけ、日本の祝日はそのまま」という訂正より）。ダークモード時は`--holiday-red-pale`のみ`html[data-palette="willow"][data-theme="dark"]`ブロックで暗い赤系に上書き、文字色の`--holiday-red`自体はライト/ダーク共通
