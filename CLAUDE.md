...
- **「すべて」表示時のみ末尾に画面高さ分の余白を確保**: 今月合わせスクロールの対象は「すべて」のみになったため、今月が12月に近く後続コンテンツが1画面に満たない場合でも今月の月カードをきっちり画面最上部に寄せられるよう、`renderCalendarList()`で`_calendarCategory === ''`の時だけ`#calendar-list`に`paddingBottom = scrollEl.clientHeight + 'px'`を動的に付与（他のカテゴリでは`''`にリセットし、無駄な余白を出さない）
