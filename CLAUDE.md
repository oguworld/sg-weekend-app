...
- **データ取得**（`scripts/fetch-life-info.js`）: RSS7件（CNA/Mothership/Straits Times/JCCI/CNA Sport〈設計書182で追加〉/Expat Living/SingaporeMotherhood〈設計書203、2026-09-14追加〉、⚠️Straits Times追加の経緯は未記録）を`rss-parser`で取得、ハイウォーターマーク方式（`data/life-info-fetch-state.json`、イベント用の状態ファイルとは分離）。Haikuで在住日本人への関連性判定＋カテゴリ付与、Sonnetで日本語要約を生成し`data/sg/life-info.json`（gitignore対象）に保存（2026-09-09設計書189で英語要約生成は廃止済み）。リテンション期間は一律7日
