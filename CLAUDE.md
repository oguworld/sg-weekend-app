...
- **「すべて」表示では日本の祝日を除外する**（2026-09-06「日本の祝日はすべてに表示しないで、カテゴリで選んだ時だけ表示」との要望より）: `renderCalendarList()`で`_calendarCategory`が空(＝「すべて」選択時)の場合、`CALENDAR_DATA`から`category==='holiday-jp'`を除外してから表示する。「日本の祝日」フィルターチップを明示的にタップした時だけ表示される（データ自体は引き続き`/api/calendar`から返るので、除外はフロントエンドの表示ロジックのみ）
