...
- **iOSプッシュ通知アイコンが旧デザインのまま表示される不具合の修正（設計書190、2026-09-10）**: `ios-app`の`@capacitor/assets`（v3系）はXcode 14+の「シングルサイズアイコン」方式に対応しており、`npx capacitor-assets generate --ios`が1024×1024の1ファイルのみを`Assets.xcassets/AppIcon.appiconset/`に生成し、ホーム画面用(60pt/76pt)はXcodeのビルドシステムが自動生成する一方、**Notifications用の20ptサイズ（40×40px=2x、60×60px=3x）は自動生成の対象に含まれず**、プッシュ通知に旧アイコンが表示され続ける原因になっていた。`.github/workflows/ios-deploy.yml`の`Generate app icons and splash screen`ステップの直後に後処理ステップを追加し、`ios-app/resources/icon.png`から`sharp`（`npm install --no-save sharp`で都度インストール、`ios-app/package.json`への永続追加はなし）で`AppIcon20x20@2x.png`/`AppIcon20x20@3x.png`を生成、`Contents.json`に`{idiom:"iphone", size:"20x20", scale:"2x"/"3x", platform:"ios"}`の2エントリを追記する方式で対応（既存のuniversal 1024×1024エントリ・ホーム画面/iPad用アイコンの生成には影響なし）。iPad用Notifications 20pt（`idiom:"ipad"`）は今回のスコープ外。**次回`release`ブランチpushによるTestFlightビルドで初めてCI環境（macOSランナー）での実際の動作確認が可能**（ローカルはLinuxのためXcodeプロジェクトを再現できずロジックシミュレーションのみで検証済み）、実機での通知アイコン表示の最終確認はユーザーに委ねる
...
- **現在はX API自動投稿ではなく「投稿下書きのLINE通知」運用**（X APIのクレジット枯渇のため停止）: `--to-line`フラグ指定時、生成した投稿文をそのままLINEに送信し、ユーザーが手動でXに貼る。実際には1日2回（`crontab -l`実測: UTC 02:00/12:00 = SGT 10:00/20:00）cron実行（以前の記載「毎日07:00/19:00」は実態とズレていたため訂正）、ログは`logs/post-to-x-draft.log`。X API送信経路自体は無変更で残置（`--to-line`を外せば復活可能）
