...
- "gourmet": 「食べる・飲む」に関わる【期間限定】の情報全般。①フードフェスティバル・フードフェア（複数店舗・屋台が集まる食のイベント）、②飲食店・カフェ・ホテルが開催する期間限定メニュー・コラボカフェ・テーマビュッフェ・ハイティー、③飲食店・食品ブランド・フードデリバリーの割引・プロモ・キャンペーン（ドーナツデー・期間限定セット・クーポン等）。食に関するものはディスカウント・プロモであってもgourmetとする。通常営業・定番メニュー紹介など常設コンテンツは含めない。
- "sale": ファッション・家電・家具・雑貨・ドラッグストア・モール全体など【非食品】小売店の割引・セール・クーポン・キャッシュバック。食品・飲食関連はsaleではなくgourmetとする。
...
1. 全く新しいお店・施設のグランドオープン（初めて営業開始）→ opening。飲食店・カフェであっても、食系プロモを同時に実施していても、opening が最優先
2. リニューアル・改装再開業・新エリア追加・新ゾーン開設は opening にしない → event またはgourmetで分類
3. 「食べる・飲む」に関わる情報（フードフェス・期間限定メニュー・食系プロモ・割引）かつグランドオープンでない → gourmet
4. 「観る・鑑賞する」が主目的（コンサート・展示・演劇など）かつグランドオープンでない → show
5. 「体験する・遊ぶ・行く・学ぶ・マーケットを楽しむ」が主目的かつグランドオープンでない → event
6. 非食品の割引・セール・クーポン（ファッション・家電・家具・雑貨など）かつグランドオープンでない → sale

【重要】opening は他のすべてのtypeより優先する。飲食店のグランドオープンは gourmet ではなく opening。セール中の新店オープンも opening。
【重要】食・飲食に関わるものはプロモ・割引であってもgourmet（opening を除く）。非食品のセールはsale。
  ...
  for (let i = 0; i < items.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      for (const r of results) {
        ...
        const validType = ['event', 'show', 'gourmet', 'sale', 'opening'].includes(r.type) ? r.type : 'event';
