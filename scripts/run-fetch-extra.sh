...
# 1日1回の取得（run-fetch-all.sh、7:00 SGT）だけでは記事がフィードから流れ落ちて
# 取りこぼされるリスクを減らすため、追加で取得を行う。
#
# 開発者向けLINE通知（notify-fetch-summary.js）: 設計書187で12:00/21:00の両方に追加し、
# 1日3回（7:00/12:00/21:00）その都度その回だけの件数を通知する方式に変更した
# （2026-09-07時点では「通知は6:30のみ、過去24時間分を合算」だったが、ユーザー要望により
# 1日3通に増えることを許容の上で「毎回・その回だけの件数」方式に戻した）。
#
# fetch-life-info.js（くらし情報、設計書183で追加）は12:00/21:00 SGTの両方で呼ぶが、
# ユーザー向けプッシュ通知（notifyContentUpdated()）はrun-fetch-all.sh（7:00 SGT）側にのみ送るため、
# このスクリプトの12:00/21:00両方とも`--no-notify`を付けて通知を抑制する（2026-09-09、ユーザー要望により
# 19:30固定から6:30固定に変更、同日中に「6:30は少し早い」との要望で7:00固定に再調整、同日中に追加実行の時刻も12:30/19:30から12:00/17:00に変更、
# さらに同日中に夕方以降の記事の取りこぼしリスクを減らすため3回目の実行を17:00から21:00に再変更）。
...
# くらし情報取得（設計書183）: ユーザー向けプッシュ通知は7:00 SGT（run-fetch-all.sh）側にのみ送るため、
# このスクリプト（12:00/21:00 SGT）は常に--no-notifyで実行する（2026-09-09変更）
