...
## リスク・未解決の質問
1. **未解決の質問**: tips展開状態は「タブを切り替えて戻ってきても展開したまま維持する」べきか「従来通り閉じた状態にリセットする」べきか、仕様として未確定。ユーザーに確認が必要
...
3. **リスク**: 新着リボンのアニメーション適用対象を「新規要素のみ」に変更することは、現状の「タブ切り替えのたびに全カードが`fadeUp`する」演出からの見た目の変化を伴う。ユーザーが意図的にこの演出を気に入っている可能性もゼロではないため、実装後に目視確認を推奨する
