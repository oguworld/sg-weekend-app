...
# 設計書75: スタンプスポット詳細モーダルのUnsplash画像が実ブラウザで読み込み失敗し続ける不具合（原因調査）

**調査日**: 2026-07-20
**担当**: investigator（コード編集なし、読み取り専用調査）
**関連**: 設計書69（スタンプラリー機能）・設計書73（画像追加）・設計書74（aspect-ratio→固定高さ修正、症状不変）

## 1. 症状の再確認

- `openStampSpotDetail()`（`public/app.js` 3949行目〜）で`#stamp-spot-detail-image`（`public/index.html` 807行目）に`imgEl.src = spot.imageUrl`を設定してもモーダル内画像が表示されない
- 設計書74で「`aspect-ratio:16/9`が原因」という仮説に基づき`height:200px`固定へ修正したが**症状は再発**（この仮説は否定された）
- 診断ログ（`imgEl.onload`/`imgEl.onerror`に`_sendDebugLog`を仕込んだもの）により、`logs/debug-nav.log`で`stamp_image_load_error`が3件記録されていることを確認済み（`isCapacitor:false`＝Web版ブラウザ、`spotId`は`merlion-park`×2、`marina-bay-sands-skypark`×1、時刻は2026-07-20 08:00:08〜08:00:17の約9秒間）
- 同時刻帯の`auth_prefs_init`ログから、このセッションは`hasToken:true`（`localStorage`に既存のログイントークンあり）＝**新規訪問者ではなく、以前からdosuru.appを利用しているリピーターのブラウザ**である可能性が高いことが判明（この点は後述の通りSW仮説の検証において重要な手がかりになる）

## 2. 事実確認の結果（検証済み・確定事項）

### 2-1. サーバー・CDN側は正常（原因ではない）
- `GET /api/stamp-spots`は正しい`imageUrl`を返す（ユーザー確認済み）
- 画像URL自体（imgix/Unsplash CDN）は、iOS Safari相当のUser-Agent・Referer・Rangeヘッダー付きなど複数パターンで検証したが、いずれも`200 OK`・`content-type: image/jpeg`・`access-control-allow-origin: *`・`cross-origin-resource-policy: cross-origin`を正常に返した（本調査で再確認済み）
- `data/sg/stamp-spots.json`の画像URL生成時刻とエラーログの時刻に大きなズレはなく、設計書57型の「CDN伝播遅延による一時403」とは考えにくい
- サーバー（`server.js`）にCSPヘッダー・Referrer-Policy等の設定は存在しない（grep確認済み）。`/api`向けのCORS・no-cacheヘッダーのみで、画像リクエストには無関係
- **新事実（CLAUDE.md未記載）**: `dosuru.app`はCloudflare経由で配信されている（`server: cloudflare`、`cf-ray`ヘッダーを確認）。ただし`images.unsplash.com`へのリクエストはブラウザから直接発行されるためこの経路とは無関係。CLAUDE.mdのインフラ記述（VPS Contabo→nginx→Express）を補足する情報として付記する

### 2-2. Service Worker登録コードは本番配信の`index.html`に存在しない（重要な前提の修正）
- `public/index.html`（本番配信版）を`grep`した結果、`navigator.serviceWorker.register(...)`の呼び出しは**存在しない**
- CLAUDE.md「PWAインストール・更新バナーの廃止（2026-07-09）」に「SW登録処理はすべて撤去済み」「`public/sw.js`本体は変更なし（Web版のオフライン対応・キャッシュ機構として残置。登録処理を削除したため**新規訪問者には未登録になる**点に注意）」と明記されている通り、コード上も確認できた（`navigator.serviceWorker.register`は`public/index-bak.html`という配信されないバックアップファイルにのみ残存）
- つまり、**2026-07-09以降にdosuru.appへ初めて訪問したユーザーのブラウザにはService Workerがそもそも登録されない**。ユーザー提示の「sw.jsのfetchハンドラが原因」という仮説が成立するのは、**2026-07-09より前からdosuru.appを訪問し続けていて、ブラウザにSWが既に登録済み・生存し続けているリピーターのケースに限られる**
- 上記2-1で確認した通り、エラーが記録されたセッションは`hasToken:true`（既存ログイン状態）であり、リピーターである可能性と矛盾しない。したがって「SWが原因」という仮説を完全には棄却できないが、「全ユーザーで再現する」という前提は誤りで、**SWが生きている一部の既存ユーザーにのみ発生しうる不具合**という限定が必要

### 2-3. `public/sw.js`のfetchハンドラの挙動精査
最後のfetchハンドラ（68〜83行目）:
```js
event.respondWith(
...
```
- `<img>`タグからのクロスオリジンGETはデフォルトで`request.mode === 'no-cors'`。SW内で`fetch(request)`にそのまま渡すと通常は`type:'opaque'`のレスポンス（`status:0`, `ok:false`）が**正常にresolve**される。`<img>`側はopaqueレスポンスでも実データがあれば正常に表示できる仕様のため、通常ケースでは問題を起こさないはず
- `images.unsplash.com`は`url.origin !== self.location.origin`かつ`fonts.googleapis.com`でもないため、`if (response.ok && ...)`の条件を満たさず`clone()`は呼ばれない。よってresponse cloneのストリーム消費競合による破損は原因として考えにくい
- **ユーザー仮説の核心部分（`.catch(() => undefined)`が発火した場合、`event.respondWith(undefined)`相当になり不正な呼び出しとしてネットワークエラー扱いになる）は技術的に正しい**。ただし、この`catch`が実際に発火する（＝`fetch(request)`自体がreject/例外を投げる）のは、通常のopaqueレスポンス取得では起きず、以下のような特殊条件が必要:
  - WKWebView（iOS Safari）のService Worker実装の既知の不安定性（SW自体の起動・休止タイミングでのfetch失敗、特にno-cors + Rangeヘッダーの組み合わせでの実装バグ報告がWebKit側に存在する。**この点は一次情報〈WebKit Bugzilla等〉による確証は取れておらず「推測」）**
  - Unsplash imgix CDN自体が`accept-ranges: bytes`を返しており、Safariが大きな画像・プログレッシブJPEGに対してRangeリクエストを発行する既知の挙動と組み合わさると、SW内fetchの失敗確率が上がる可能性がある（**推測**）
  - ネットワーク遮断・SW自体の突然の終了（iOSのメモリ回収等）のタイミングと重なった場合
- 上記はいずれも「起こりうる」レベルの技術的可能性であり、**今回のログだけでは`.catch`分岐が実際に発火したかどうかまでは確認できていない**（`stamp_image_load_error`は`imgEl.onerror`の発火のみを記録しており、SWの`.catch`発火とブラウザ側`<img>`の`onerror`発火を紐づける追加ログが無いため）

### 2-4. 既存Unsplash画像使用箇所（イベントカード・コース詳細・マイコースカード）との実装差分（重要な新知見）
コードを比較した結果、**スタンプスポット詳細画像だけが他3箇所と実装パターンが異なる**ことを確認した:

| 箇所 | 行番号 | 実装方式 |
|---|---|---|
| イベントカード画像 | `public/app.js` 1355行目付近 | `innerHTML`テンプレート文字列内で`<img src="${e.image}">`を**新規生成** |
| コース詳細画像 | `public/app.js` 4411行目付近（`renderCourseDetail()`） | `innerHTML`テンプレート文字列内で`<img src="${course.imageUrl}">`を**新規生成** |
| マイコースカード画像（生成直後プレビュー） | `public/app.js` 4861行目付近（`renderCourseResultHtml()`） | `innerHTML`テンプレート文字列内で`<img src="${course.imageUrl}">`を**新規生成** |
| **スタンプスポット詳細画像** | `public/app.js` 3967〜3984行目（`openStampSpotDetail()`） | **静的HTMLに既に存在する`<img id="stamp-spot-detail-image" style="display:none">`要素に対し、後から`.src`プロパティを代入** |

- 既存3箇所は「新規に生成された`<img>`要素がDOMに挿入されると同時に`src`が確定している」パターンで、ブラウザはパース直後に即座にリクエストを発行する。ユーザー提示の情報によれば、**これら3箇所は本番で正常表示されていると推測される**（今回の不具合報告がスタンプ機能のみに限定されていること自体が状況証拠。ただし本調査ではこの3箇所についてユーザーからの動作報告や別ログでの直接確認はできておらず、「同じSWコードパスを通るため理論上同じリスクを抱えている」という位置づけ。**確定的な検証はできていない＝推測**）
- スタンプスポット詳細画像は「`display:none`の既存要素に、モーダルを開くたびに`src`を再代入する」というライフサイクルの違いがある。この方式固有のリスクとして:
  1. 同一スポットを連続して開き直した場合（ログの`merlion-park`が2回連続でエラーになっている状況と一致）、**2回目以降は`src`に同一URLを再代入することになり、ブラウザによっては`load`/`error`イベントが発火しない、あるいはキャッシュされた失敗結果を即座に再利用してしまう既知の挙動**がある（Safari/WebKitでは特に顕著、`img.src`への同一URL再代入は仕様上「何もしない」扱いになるケースがある）。1回目の失敗原因が仮に一過性のもの（SWの過渡的な不具合等）だったとしても、2回目に「再試行」にならず「前回の失敗を再現するだけ」になっている可能性がある
  2. `display:none`要素はレイアウトツリーから除外されるため、一部のブラウザ実装（特に古いWebKit系）では非表示要素への画像読み込み優先度が下げられたり、遅延評価される場合がある（**推測、一次情報未確認**）
  3. `onload`/`onerror`をJSプロパティで**毎回re-assign**している（3970〜3974行目）。これ自体はHTML属性の`onerror`を上書きするだけで問題ない設計だが、`imgEl.src = spot.imageUrl`の代入前にハンドラを設定している点は正しい順序（代入後にハンドラをつけると発火を取りこぼすリスクがあるため、現状の順序自体は問題ない）

### 2-5. onclick二重発火（ゴーストクリック）の可能性は低い
- `openStampSpotDetail()`の呼び出し元は、マップマーカー（`marker.on('click', ...)`、Leaflet独自のクリックイベント）とリストカード（`onclick="openStampSpotDetail('${spot.id}')"`、HTML属性）の2箇所（`public/app.js` 3849行目・3932行目）
- CLAUDE.md記載の「onclick属性＋touchendハンドラの二重登録とゴースト遅延クリック」パターンについて、`stamp`関連要素専用の`touchend`ハンドラ登録は`grep`で確認した限り存在しない。したがって、この既知のゴーストクリック機構がスタンプスポットカードの二重発火を引き起こしている可能性は**低い**（ただしLeafletマーカー自体が内部的にtouch/clickの両方を処理する実装になっている可能性はゼロではなく、完全には否定できない）
- ログの3件（08:00:08, :14, :17）は数秒間隔であり、瞬間的な二重発火というよりは、ユーザーが複数スポットを順に開いて確認している自然な操作ログとも解釈できる

## 3. 原因の特定（結論）

**不明（複数の仮説があるが、いずれも一次情報による確証・再現実験ができておらず断定できない）**

現時点で確度が高いと考えられる仮説を確度順に並べる:

1. **（確度：中〜高、ただし推測）Service Workerが介在するケースに限定されるが、その中でも`fetch(request)`の`.catch`分岐が実際に発火する特殊条件（WKWebView/Safari固有のno-cors+Range処理の不安定性等）が引き金になっている可能性**。ただしこの不具合はSWが生きている一部の既存訪問者のみに影響し、「全員で再現する」不具合ではないため、報告されている症状の再現性の高さ（3件連続）と完全には整合しない可能性がある
2. **（確度：中、推測）`display:none`要素への`src`再代入という設計自体が、既存3箇所（新規`<img>`生成方式）とは異なるリスクを抱えている**。特に同一スポットを連続して開いた場合の再代入時の挙動（`merlion-park`が2回連続でエラーだった事実と整合する）
3. **（確度：低、未検証）imgix CDNのSINエッジでの一時的な不調・レート制限**。curlでの検証はできているが、実際のユーザー環境（地域・ネットワーク経路）とは異なるため完全には否定できない

**上記いずれも、現在のログ（`imgEl.onload`/`onerror`のみ）では原因を一意に特定できるだけの情報が不足している。** 特に「SWの`.catch`が発火したかどうか」「同一URLへの2回目の`src`代入だったか（初回訪問か再訪問か）」を切り分けるログが存在しない。

## 4. 修正方針

### 4-1. 最優先: Service Workerの関与を構造的に排除する（ユーザー提示案を採用）
`public/sw.js`の最後のfetchハンドラで、クロスオリジンリクエスト（`url.origin !== self.location.origin`かつ`url.hostname !== 'fonts.googleapis.com'`）は一切インターセプトせず、ハンドラの先頭で早期`return`し、ブラウザのネイティブfetchに完全に委ねる。

- 理由: 現状の実装はクロスオリジンレスポンスを`caches.match`→`fetch`→（キャッシュ条件を満たさないため）そのまま返す、という**キャッシュ機構として何のメリットもない**単なる中継になっている（`response.ok && url.origin === self.location.origin`の条件によりクロスオリジンは絶対にキャッシュされない）。中継する意味がないにもかかわらず、SWのfetch実装の不安定性リスク（2-3節）だけを負っている状態
- この変更は「SWがそもそも原因かどうか確証がない」状態でも、**副作用がほぼゼロ（クロスオリジン画像は元々SWでキャッシュされていないため機能的な後退がない）でリスクだけ下げられる**ため、原因が完全に特定できていなくても実施する価値が高い
- 影響範囲: `images.unsplash.com`だけでなく、他の全クロスオリジンリクエスト（Instagram embed等）にも同じ扱いが及ぶが、これらも同様に現状キャッシュされていないため実質的な機能変化はない

### 4-2. 表示方式を既存3箇所と統一する（構造的な差分の解消）
`openStampSpotDetail()`の画像表示を、既存の「新規`<img>`生成」パターンに寄せる。具体的には、静的`<img>`要素への`.src`再代入ではなく、モーダルを開くたびに画像コンテナのHTMLを`innerHTML`で作り直す（他3箇所と同じ実装パターンに統一）ことで、同一URL再代入時の既知のブラウザ挙動リスクを回避する。

- これにより「モーダルを開き直すたびに新しい`<img>`要素がゼロから生成される」ことになり、2-4節で指摘した「同一URL再代入時にload/errorが発火しない」問題が構造的に起きなくなる
- 既存3箇所と実装を統一することで、今後同種の不具合調査をする際の一貫性も上がる（CLAUDE.md「UIスタイル規約」の精神に合致）

### 4-3. 診断ログの継続・強化（原因の最終特定のため）
現状の`stamp_image_load_success`/`stamp_image_load_error`に加え、以下を一時的に追加することを推奨する（実装時に検討）:
- `spot.imageUrl`が前回開いた時と同一かどうか（`_stampSelectedSpot`との比較）を記録し、「初回オープン失敗」か「再オープン失敗」かを区別できるようにする
- `navigator.onLine`・`performance.now()`（リクエスト開始からerror発火までの時間）を記録し、瞬時に失敗している（＝SW等の即時エラー）のか、タイムアウト的に失敗している（＝ネットワーク遅延）のかを区別できるようにする

### 4-4. 実施しないこと（スコープ外）
- imgix CDN側の設定変更（Unsplash側の管轄のため対応不可）
- Service Worker自体の完全廃止（Web版のオフライン対応・キャッシュ機構としての役割はCLAUDE.mdで明記されている既存機能のため、今回の不具合調査の範囲を超える）

## 5. 変更するファイル一覧（修正実施時の想定、今回は未実施）

- `public/sw.js`: クロスオリジンリクエストの早期return追加（4-1）
- `public/app.js`: `openStampSpotDetail()`の画像表示を`innerHTML`生成方式に変更（4-2）、診断ログの強化（4-3、任意）
- `public/index.html`: `#stamp-spot-detail-image`を静的`<img>`要素から、画像を差し込むための空コンテナ（`<div id="stamp-spot-detail-image-container">`等）に変更する可能性（4-2の実装方式次第）
- キャッシュバスティング（`index.html` app.js?v=、`sw.js` CACHE_NAME=）は既存ルール通りセットで実施

## 6. 受け入れ基準（修正後にこうなればOK）

1. TestFlightビルド後、iOS実機・Web版Safari両方で、スタンプスポット詳細モーダルの画像が安定して表示され続けること（一瞬表示されて消える、あるいは最初から表示されない、のいずれの症状も再発しないこと）
2. 同一スポットを連続して複数回開いた場合でも、毎回画像が表示されること
3. `logs/debug-nav.log`で`stamp_image_load_error`が記録されなくなること（またはゼロに近い頻度になること）
4. 既存3箇所（イベントカード・コース詳細・マイコースカード）の画像表示に回帰がないこと
5. Service Worker関連の他機能（オフライン対応、静的アセットキャッシュ）に回帰がないこと

## 7. 再発防止策

1. **外部（クロスオリジン）リソースを表示する新機能を追加する際は、Service Workerのfetchハンドラが対象リクエストをどう扱うか（インターセプトの要否）を実装前に必ず確認する。** 現状の`sw.js`は「クロスオリジンは`self.location.origin`と`fonts.googleapis.com`以外キャッシュしない」という条件はあるが、「インターセプトするかどうか」自体の判断が欠落しており、キャッシュされないにもかかわらず中継だけする無意味かつリスクのある経路になっていた。今後別の外部画像・APIを扱う機能を追加する際も同じ穴に落ちないよう、`sw.js`のfetchハンドラの対象範囲（`/api/`・navigate・その他）を実装前に確認する
2. **既存の類似実装パターン（今回で言えば「他3箇所のUnsplash画像表示」）がある場合は、新機能もそのパターンに合わせて実装する。** 独自の実装方式（静的要素への`src`後代入）を選ぶと、既存パターンでは踏まなかった別のブラウザ挙動リスク（同一URL再代入時の挙動等）を新たに背負うことになる。CLAUDE.md「既存のデザインパターンを踏襲する」の原則はUIスタイルだけでなくDOM操作の実装方式にも適用されるべき
3. **「表示されない」系の不具合調査では、CSS（設計書74で先に試みた`aspect-ratio`修正）だけでなく、必ずService Worker・キャッシュ層・DOM生成方式の3点をセットで疑う。** 今回、CSS修正で症状が再発したことで初めてSW・実装差分の調査に進んだが、`logs/debug-nav.log`の`onerror`ログ（＝ネットワークリクエスト自体の失敗）があれば、CSS要因は最初から除外できた。今後は診断ログを先に仕込んでから原因切り分けを開始する順序を徹底する
4. **同一箇所への複数回の値代入（`img.src`の再代入等）を伴うUI要素を実装する際は、「初回」と「2回目以降」で挙動が変わりうることを設計時から意識する。** 静的要素の再利用は一見効率的に見えるが、ブラウザのリソース読み込みキャッシュ・イベント発火の仕様は「新規要素」を前提にしていることが多く、既存の確立されたパターン（新規`<img>`生成）から意図的に外れる場合は追加のリスク評価が必要

## 承認状況

2026-07-20 debuggerによる原因調査。単一の確定原因は特定できなかったが、§4-1（Service Workerでクロスオリジンリクエストを早期returnし関与させない）・§4-2（スタンプ詳細画像を既存3箇所と同じ`innerHTML`新規生成方式に統一）・§4-3（診断ログの強化、任意）を修正方針として**ユーザー承認済み**。§4-4のスコープ外事項は対象外のまま。実装後は受け入れ基準（§6）に沿って`logs/debug-nav.log`での`stamp_image_load_error`解消を確認すること。

---

# 設計書76 — 「コースの画像表示されなくなりました」報告の原因調査（設計書75デプロイ直後）

（2026-07-20 investigatorによる読み取り専用の原因調査。コード変更は一切行っていない。設計書75〈スタンプスポット詳細画像の恒久修正、コミット`4549c2f`〉デプロイ直後のユーザー報告を受けた調査で、設計書69〜75の未解決事項の延長線上にある内容のため、既存設計書は削除・上書きせず本設計書として追記する）

## 1. 症状

ユーザー報告: 「コースの画像表示されなくなりました」。設計書75（Service Worker早期return＋`openStampSpotDetail()`のinnerHTML新規生成方式への統一、pm2 restart済み）デプロイ直後の発生。

## 2. 「コースの画像」が指すものの特定（最重要の切り分け）

調査の結果、**ユーザーが言う「コース」画面は、実際には設計書71で初期表示タブが変更された結果、デフォルトで「スタンプラリー（スタンプマップ）」が表示される状態になっている**ことを確認した。

- `public/index.html` 145行目: コース画面の見出し自体が`courseScreenTitle`＝「スタンプラリー」（設計書73で変更）
- `public/index.html` 148〜150行目: タブ構成は「スタンプラリー(`map`)／みんなのコース(`everyone`)／マイコース(`mylist`)」の3タブで、**`map`が一番左＝デフォルト表示**（設計書71・72で確定）
- `public/index.html` 511行目: ボトムナビの表示ラベル自体も`navCourse`＝「制覇」に変更済み（設計書72）
- したがって、ユーザーが「コース」タブ（ボトムナビ「制覇」）を開いた際に最初に目にするのは、**既存のAIコース機能（人気/みんなのコース/マイコース）ではなく、新規のスタンプラリー機能**である

一方、**既存のAIコース機能（`renderCourseDetail()` 4414行目、`renderCourseResultHtml()` 4863行目、マイコースカード等）の画像描画コードは、設計書75の差分に一切含まれていない**ことをgit diffで確認済み（後述§3）。

**結論（推測を含む）**: ユーザーが報告した「コースの画像」は、文言上は既存のAIコース機能を指しているように読めるが、実際にその画面を開いた際に見えている・画像が表示されないと感じているのは、設計書73〜75で継続して未解決の「スタンプスポット詳細画像」である可能性が高い（**推測**。ユーザー本人にどちらの画面かの再確認を強く推奨）。以降はこの仮説に基づき、両方の可能性を分けて調査した結果を記載する。

## 3. 既存AIコース機能（人気/みんなのコース/マイコース）の画像表示への影響調査

### 3-1. コード差分の確認（影響なし）
`git show 4549c2f --stat`で変更ファイルは`public/app.js`（`openStampSpotDetail()`関数内、3964〜3986行目のみ）・`public/sw.js`・`public/index.html`（`#stamp-spot-detail-image`関連、804行目・1080行目のみ）・`CLAUDE.md`・`.claude/plan.md`。

`renderCourseDetail()`（4414〜4421行目、`course.imageUrl`をテンプレートリテラルで直接`<img src="...">`として出力）、`renderCourseResultHtml()`（4863〜4866行目、同様のテンプレートリテラル方式）、マイコースカード（4245行目・4288行目・4326行目、同様の方式）は、いずれも設計書75の差分に一切含まれていない。`escapeHtml()`（`public/app.js` 915行目）も既存関数であり、`openStampSpotDetail()`内の新規使用箇所（`escapeHtml(displayName)`）で`ReferenceError`が起きる可能性はない（`node --check`もcheckerで既にクリア済み、`.claude/next.md`記載の通り）。

### 3-2. データ側の確認（問題なし）
`curl "https://dosuru.app/api/courses?city=sg&tab=popular"`で取得した5件全てに正常な`imageUrl`（Unsplash URL）が含まれることを確認済み。データ側の破損はない。

### 3-3. 画像URL自体の疎通確認（問題なし）
上記`imageUrl`のうち1件（`https://images.unsplash.com/photo-1553771620-...`）に対し`curl -sI`で`HTTP/2 200`を確認済み。CDN側の障害でもない。

### 3-4. Service Worker変更の影響評価（影響なし、むしろ変更前よりシンプルな経路になった）
`public/sw.js`の変更前（`git show 4549c2f~1:public/sw.js`）と変更後を比較した。

- **変更前**: 全リクエスト（クロスオリジン含む）が最後の「その他の静的アセット: Cache-first」分岐（78〜91行目）で`event.respondWith(caches.match(request).then(...))`により処理されていた。`caches.match(request)`がミスした場合`fetch(request)`し、`response.ok && url.origin===self.location.origin`（またはfonts.googleapis.com）の条件を満たさないUnsplash画像等はキャッシュされないが、レスポンス自体は正常にブラウザへ返っていた（＝SWが「意味のない中継」をしていただけで、機能的には問題なかった）
- **変更後**: `url.origin !== self.location.origin && url.hostname !== 'fonts.googleapis.com'`の条件に一致するリクエスト（Unsplash画像を含む）は、fetchハンドラの先頭（52〜54行目）で`event.respondWith()`を一切呼ばずに`return`する。これにより該当リクエストはService Workerの介在なしにブラウザのネイティブfetchで処理される
- 条件式`url.origin !== self.location.origin && url.hostname !== 'fonts.googleapis.com'`に論理的な誤り（AND/OR取り違え等）は見当たらない。Unsplash画像（`images.unsplash.com`）はどちらの条件も満たす（origin不一致かつfonts.googleapis.comでもない）ため、正しく早期returnの対象になる
- **結論**: この変更はコース画像・スタンプ画像を問わず、外部画像リクエストへのSW関与を「ある（が無意味）」から「ない」に変えただけであり、機能を壊す方向の変更ではない。むしろSW経由の不安定性リスクを除去する意図の変更であり、新規の不具合を生む可能性は低いと判断する

### 3-5. 本番デプロイの反映確認
- `curl https://dosuru.app/` → `app.js?v=20260720f`（設計書75の想定通り）
- `curl https://dosuru.app/sw.js` → `CACHE_NAME = 'sg-weekend-v635'`（設計書75の想定通り）
- 差分通りに本番配信されていることを確認済み

### 3-6. pm2ログの確認
`pm2 logs sg-weekend --lines 80 --nostream`で確認した範囲では、設計書75デプロイに起因すると考えられる新規エラーは見当たらなかった。表示されたエラー（`ERR_ERL_UNEXPECTED_X_FORWARDED_FOR`、express-rate-limitの`trust proxy`設定警告）は今回の変更と無関係の既存の警告と考えられる（**推測、express-rate-limit関連の設定は設計書75の変更対象外で、reverse proxy設定に起因する既存の警告と見られる**）。

**§3のまとめ**: 既存AIコース機能の画像表示は、コード・データ・CDN疎通・SW変更影響・本番反映のいずれの観点からも異常が確認できなかった。**「コース」＝既存AIコース機能を指すという解釈であれば、原因は「不明」**（現時点で不具合を再現する具体的な経路が見当たらない）。

## 4. スタンプスポット詳細画像（設計書73〜75で継続する既知の未解決問題）の調査

### 4-1. `logs/debug-nav.log`の確認（重要な事実）
設計書75デプロイ後（本番反映確認済みの`09:39`以降）のログにも、`stamp_image_load_error`が複数記録されている:

```
{"receivedAt":"2026-07-20T09:39:20.877Z","event":"stamp_image_load_error","spotId":"merlion-park", ...}
{"receivedAt":"2026-07-20T09:39:23.611Z","event":"stamp_image_load_error","spotId":"marina-bay-sands-skypark", ...}
{"receivedAt":"2026-07-20T09:55:34.022Z","event":"stamp_image_load_error","spotId":"merlion-park", ...}
{"receivedAt":"2026-07-20T09:55:41.877Z","event":"stamp_image_load_error","spotId":"gardens-supertree", ...}
{"receivedAt":"2026-07-20T09:55:44.154Z","event":"stamp_image_load_error","spotId":"singapore-zoo", ...}
{"receivedAt":"2026-07-20T09:55:45.952Z","event":"stamp_image_load_error","spotId":"marina-bay-sands-skypark", ...}
```

いずれも`"isCapacitor":false`（**Web版ブラウザからのアクセス**、iOSアプリ内ではない）。すべてのエントリで`stamp_image_load_success`（成功ログ）が一切記録されておらず、**設計書75の修正実施後も、複数の異なるスポットで画像読み込みが継続的に失敗している**ことが確認できる。

これは設計書75の`.claude/next.md`記載の要フォロー事項「③`logs/debug-nav.log`で`stamp_image_load_error`が記録されなくなること」が**満たされていない**ことを意味する。

### 4-2. 該当画像URLのサーバー側疎通確認（問題なし、curlレベルでは200）
ログに記録された2つのURL（`merlion-park`・`marina-bay-sands-skypark`）に対し`curl -sI`を実行したところ、いずれも`HTTP/2 200`、`content-type: image/jpeg`、`access-control-allow-origin: *`、`cross-origin-resource-policy: cross-origin`が正常に返っている。Referer/Originヘッダーを付与したcurlでも結果は同じ。**サーバー側・CDN側・CORS/CORPヘッダーレベルでは異常が見当たらない**。

### 4-3. サーバー側CSPヘッダーの確認（該当なし）
`server.js`・`public/index.html`いずれにも`Content-Security-Policy`ヘッダーの設定は見当たらない。CSPによる画像ブロックの可能性は排除できる。

### 4-4. コード上の疑わしい点（推測、未確証）
`openStampSpotDetail()`（`public/app.js` 3949〜4001行目）の画像挿入処理を確認したところ、以下の順序になっている:

1. 3970〜3986行目: `imgContainer.innerHTML = ...`で`<img>`を新規生成し、`onload`/`onerror`をアタッチ（**この時点で親要素`#stamp-spot-detail-sheet`はまだ`.visible`クラスなし＝`display:none`**）
2. 3995〜3996行目: `#stamp-spot-detail-overlay`・`#stamp-spot-detail-sheet`に`.visible`クラスを付与（ここで初めて`display:flex`になる）

`.plan-modal`（`public/app.css` 2706行目）は`display:none`がデフォルト、`.plan-modal.visible`（2712行目）で`display:flex`になる。つまり**`<img>`要素の生成・`src`設定が、モーダルがまだ非表示（`display:none`）の状態で行われている**。

一般的なブラウザの挙動としては`display:none`の祖先を持つ`<img>`でも`src`が設定されればリソース取得（fetch）自体は行われることが多いが、**この実装順序自体が`load`/`error`イベントのタイミング・信頼性に何らかの影響を与えている可能性は完全には排除できない**（**推測**。決定的な証拠は今回の調査範囲〈curlでの疎通確認・コードリーディング〉では得られなかった。ブラウザの開発者ツールのNetworkタブでの実機・実ブラウザでの直接観測が必要）。

なお、この「モーダル非表示中に画像を挿入し、後から表示状態にする」という順序自体は、既存の他3箇所（`renderCourseDetail()`等）と比較しても大きな差はない（`renderCourseDetail()`もモーダル本体の`innerHTML`を構築してから`document.getElementById('course-detail-overlay').style.display = 'block'`する順序であり、同様に「非表示状態での`<img>`生成→表示化」というパターン自体は共通している）。そのためこの順序の違いだけでは、スタンプ詳細画像だけが失敗し続けている理由の説明として不十分である。

### 4-5. 設計書74・75で既に試みられた修正が効いていない事実
設計書74（`aspect-ratio`→固定高さ修正）、設計書75（SW早期return＋innerHTML新規生成方式統一）と、2回にわたる修正がいずれも本質的な解決に至っていない。これは、**投稿した仮説（CSS起因、静的要素への`src`再代入起因、SWの不安定な中継起因）のいずれも「真因ではなかった」または「真因の一部にすぎなかった」ことを示す実測結果**である。

**§4のまとめ**: スタンプスポット詳細画像の読み込み失敗は、設計書75の対策後も継続している。サーバー・CDN側は正常なため、原因はクライアント側（ブラウザの実行環境・タイミング・ネットワーク条件のいずれか）にあると推測されるが、**今回の調査範囲（読み取り専用のコードリーディング＋curl検証）では確定的な原因を特定できなかった。原因は「不明」**とする。

## 5. Web版とiOS版の切り分け（重要な事実）

`git log main ^release`で確認したところ、設計書69〜75（スタンプラリー機能一式、コミット`793f07a`〜`4549c2f`の7コミット）は**すべてローカル`main`ブランチにのみ存在し、`release`ブランチには未マージ**（`.claude/next.md`にも「releaseブランチへのpush・TestFlightビルドは今回ユーザーから明示指示が出ていないため未実施」と明記済み）。

- **iOS App Store版（TestFlight/本番）には、スタンプラリー機能自体がまだ配信されていない**
- `logs/debug-nav.log`の`stamp_image_load_error`はすべて`"isCapacitor":false`＝**Web版（dosuru.app、テスト環境）からのアクセス**
- したがって、ユーザーが今回「コースの画像表示されなくなりました」と報告した経路は、**Web版（dosuru.app）をブラウザで開いての確認である可能性が高い**（**推測**。ユーザーがどの環境〈Web版ブラウザ/iOS実機TestFlight〉で確認したかは未確認のため、原因調査の観点から本人に確認することを強く推奨する）

## 6. 総合判断

1. **既存AIコース機能（人気/みんなのコース/マイコース）の画像表示自体には、設計書75による回帰の証拠は見当たらない**（コード差分・データ・CDN疎通・SW変更影響のいずれも問題なし）
2. **「コースの画像」報告の実態は、設計書71でデフォルト表示に変更された「スタンプラリー」機能内のスポット詳細画像である可能性が高い**（推測。ユーザーへの確認を推奨）
3. **スタンプスポット詳細画像の読み込み失敗は、設計書75の対策実施後もdebug-nav.logで継続確認されており、未解決のまま**。真因は依然として「不明」
4. 仮に3.が正しいとすると、「設計書75をデプロイした直後に画像が表示されなくなった」というユーザーの言い方は**時系列の誤解の可能性がある**（**推測**）。実際には設計書73（画像追加の初回実装）の時点から一貫して同じ症状が続いており、設計書75は「直したはずが直っていない」状態。ユーザーからすれば「最近のデプロイ後に気づいた／改めて確認したら直っていなかった」という体験として「表示されなくなった」と表現した可能性がある

## 7. 修正方針（提案、未実装）

### 7-1. 最優先: ユーザーへの事実確認（コード変更ではなく、まず行うべきこと）
以下2点をユーザーに確認しないと、これ以上の的確な原因調査・修正は困難:
1. 「コースの画像」とは、①ボトムナビ「制覇」を開いた際のスタンプラリー機能のスポット詳細画像か、②「みんなのコース」「マイコース」タブのAIコース画像か、どちらを指しているか
2. 確認した環境は、Web版（ブラウザでdosuru.appを開いた）か、iOS実機（TestFlightアプリ）か（**iOS実機の場合、そもそもスタンプラリー機能自体が未配信のため、①の解釈は成立しない。その場合は既存AIコース機能側の問題として再調査が必要になる**）

### 7-2. ①（スタンプラリー機能）だった場合の追加調査方針
- 実ブラウザ（Chrome DevTools等）のNetworkタブで、`stamp-spot-detail-image-container`内の`<img>`リクエストが実際にどのステータスで失敗しているか（CORS拒否／タイムアウト／DNSエラー／HTTPエラーコード等）を直接観測する。現状のcurl検証だけでは「サーバー側は200を返す」ことしか分からず、ブラウザ固有の失敗理由（例: 同時多重リクエストによるブラウザ側の同時接続数制限、`img`要素の`referrerpolicy`未指定によるUnsplash側の何らかの制限、iOS WKWebView固有の挙動等）は特定できていない
- `imgEl.onerror`ハンドラ内で、現状記録していない追加情報（`navigator.onLine`、`performance.now()`との差分、リトライ時の成否）を一時的に計装し、`logs/debug-nav.log`で傾向を追う
- `<img>`生成タイミングをモーダルの`.visible`クラス付与後に変更した場合に症状が変化するか検証する（§4-4で述べた「非表示状態での生成」という違いの影響を実験的に切り分ける）

### 7-3. ②（既存AIコース機能）だった場合の追加調査方針
今回の調査では既存AIコース機能側に問題を発見できなかったため、②だった場合は改めてユーザーから「画面のどの部分の画像が表示されないか」「エラーメッセージや空白の見え方」等の詳細なスクリーンショット・状況を取得し、再調査が必要（原因不明のまま）。

## 8. 変更するファイル一覧

今回は調査のみのため変更なし。§7-2の追加調査を実施する場合の想定対象ファイル: `public/app.js`（`openStampSpotDetail()`の計装追加・生成タイミング変更の実験）。

## 9. 受け入れ基準（次回対応時）

1. ユーザーから「コースの画像」がどちらの機能を指すか、および確認環境（Web版/iOS実機）の回答を得ること
2. ①の場合: 実ブラウザのNetworkタブでの直接観測により、画像リクエスト失敗の具体的な理由（ステータスコード・エラー種別）を特定すること
3. 特定した原因に対する修正を実施し、`logs/debug-nav.log`で`stamp_image_load_error`が解消されることを確認すること

## 10. 再発防止策

1. **UI文言変更（設計書71〜72で「コース」画面のデフォルトタブ・ボトムナビラベルを「スタンプラリー」「制覇」に変更したこと）によって、ユーザーが同じ言葉（「コース」）で指す対象が変わりうることを、報告受付時に意識する。** 画面名・ボトムナビラベルの変更を伴う機能追加を行った直後は、ユーザーからの不具合報告の文言が旧来の呼び方のまま送られてくる可能性が高く、「どの画面のことか」を最初に確認するステップを省略しないこと
2. **未リリース（TestFlight未実施）の新機能に関する不具合報告を受けた際は、報告者がどの環境（Web版/iOS実機）で確認したかを必ず最初に確認する。** リリース状況（`release`ブランチとの差分）を確認しないまま調査を進めると、iOS実機で発生しえない事象を前提に無駄な調査をするリスクがある
3. **同じ症状に対して複数回（設計書74→75）修正を試みても直らない場合、次の修正に進む前に、必ず「実ブラウザでの直接観測（DevTools Network/Console）」を修正着手条件に加えることを検討する。** サーバー側のcurl検証だけでは、ブラウザ固有の挙動（読み込みタイミング、同時接続数制限、`display:none`祖先での挙動等）に起因する問題を確定できない限界がある。可能であればユーザーに「開発者ツールのコンソール・ネットワークタブのスクリーンショット」の提供を依頼する運用を検討する

## 承認状況

2026-07-20 investigatorによる読み取り専用の原因調査。コード変更は一切行っていない。**単一の確定原因は特定できなかった**。「コースの画像」がスタンプラリー機能・既存AIコース機能のどちらを指すか、および確認環境（Web版/iOS実機）をユーザーに確認することを最優先の次アクションとして提案する。承認待ち。

## 11. 最終原因判明（2026-07-20、ユーザーへの直接確認により解決）

メインエージェントがユーザーに確認環境を質問した結果、以下が判明した:
- 確認環境は**Web版（Safari等でdosuru.appを開いていた）**。「コースの画像」は本節の推測通り、スタンプラリー機能のスポット詳細画像を指していた（ボトムナビ「コース」→「制覇」への改称、既存AIコース機能ではない）
- **根本原因はコード側の不具合ではなく、ユーザーのiPhoneで有効になっていたVPNが、Unsplash CDN（`images.unsplash.com`）へのリクエストを妨げていたことだった**。ユーザーがVPNを切った結果、画像が正常に表示されるようになったことを確認済み
- curlによるサーバー・CDN側の検証（設計書74・75で実施）が常に成功していた理由が、これで完全に説明できる（curlはVPNを経由しないため）
- 設計書74（`aspect-ratio`→固定高さ）・設計書75（Service Worker早期return・`innerHTML`新規生成方式への統一）で行った修正自体は、原因ではなかったものの、既存の確立されたパターンへの統一という点で無駄ではなく、**ロールバックする必要はない**（コードの一貫性・堅牢性の観点で維持する）
- 設計書74・75で追加した診断ログ（`stamp_image_load_success`/`stamp_image_load_error`、`_sendDebugLog`経由）は、原因確定に伴い**役目を終えたため削除する**（CLAUDE.md記載の「計装は原因確定後に削除してよい使い捨てのもの」という運用方針に従う）

### 再発防止策の追記
4. **「サーバー側は正常なのに実機だけ失敗する」系の不具合では、ユーザー環境要因（VPN・Private Relay・広告ブロッカー・キャリアの通信最適化プロキシ等）を疑うタイミングを早める。** 今回は設計書74・75と2段階のコード側修正を試みてから初めてユーザーに環境要因を確認したが、「サーバー・CDN側は正常」「特定の1URLだけでなく全スポットで一貫して失敗」という状況証拠が揃った時点（設計書75の時点）で、コード修正より先にユーザー環境（VPN等）の確認を行うべきだった
...
# 設計書85 — バッジ画像の透明化修正（チェッカーボード焼き込みバグ）＋Centralエリアバッジの画像差し替え

（2026-07-21 メインエージェントが直接調査・プロトタイプ検証済み。実装内容が明確な画像アセット修正のためplannerを介さずこの場で設計書化。コード実装はorchestratorに依頼する）

## 1. 背景

ユーザーから「スタンプは丸くくり抜けるはずなんだけど実際はどう？」との指摘を受け、`.stamp-level-complete-badge-img`（設計書83〜84、全制覇バッジの画像）を調査したところ、想定と異なり四角いチェッカーボード柄ごと表示されていることが判明した。

調査の結果、以下2件の不具合が見つかった。

### 1-1. 全10枚のバッジ画像が透明背景になっていない（本質的なバグ）

設計書80・81は「256×256px・透明背景」と記録していたが、実際には**全10枚とも透明なピクセルが1つも存在しない（アルファチャンネルが全ピクセル255=完全不透明）**ことを`sharp`でアルファヒストグラムを取得し確認した。

```
badge-central.png ... unique alpha values: 1 (FULLY OPAQUE)
badge-east.png ... 同上
badge-west.png ... 同上
badge-north.png ... 同上
badge-north-east.png ... 同上
badge-sentosa.png ... 同上
badge-level-standard.png ... 同上
badge-level-local.png ... 同上
badge-level-niche.png ... 同上
badge-level-special.png ... 同上
```

Nano Bananaが「透明背景」を生成した際、実際には画像編集ツールが透明領域のプレビュー表示に使う市松模様（チェッカーボード、白と薄灰色の格子、観測値は概ね(255,255,255)と(199〜204,199〜204,199〜204)の交互パターン）を**本物のピクセルとしてそのまま焼き込んで**出力していた。CSS側（`.stamp-level-complete-badge-img`等）は`object-fit:contain`のみで円形マスクは掛けていないため、この市松模様の正方形がそのまま見えてしまっていた。

### 1-2. `badge-central.png`が別の画像に差し替わっていた（無関係の別バグ）

透明化調査の過程で全10枚を目視確認したところ、`badge-central.png`（Centralエリアバッジ、本来はマーライオンのイラスト）が**Pikmin Bloomのバッジ画面のスクリーンショット**（ユーザーが設計書80検討時に参考画像として送った別の画像）になっていることが判明した。`git log -- public/images/stamp-badges/badge-central.png`で確認したところ、設計書80のコミット（`4972da0`）時点から一貫してこの誤った画像のままであり、今回のセッション作業で壊れたものではない。他9枚（East/West/North/North-East/Sentosa・レベル4種）は目視確認の結果いずれも正しいイラストだった。

ユーザーに報告したところ、正しいマーライオンのイラスト（1024×1024px、Nano Banana生成、同じくチェッカーボード焼き込みあり）を新たに送付してもらい、差し替え用画像として確定した。

## 2. 確定済み修正方針（メインエージェントがプロトタイプ検証済み・ユーザー承認済み）

### 2-1. チェッカーボード自動透明化（ユーザー選択: 3択中「自動で透明化」を選択）

以下のアルゴリズムをNode.jsスクリプト（`sharp`使用）でプロトタイプし、視覚的に確認済み。

1. **境界からのフラッドフィル**: 画像の四辺（x=0/x=w-1/y=0/y=h-1の全ピクセル）を起点に、「チェッカーボード色」と判定されたピクセルをアルファ0（透明）にしながら4方向に伝播する。チェッカーボード色の判定基準: `max(R,G,B) - min(R,G,B) <= 30`（無彩色＝グレースケール判定）かつ`max(R,G,B) >= 140`（明るい色のみ、暗い部分は対象外）。この閾値は実際のバッジ画像のドロップシャドウ部分（コインの縁の下に描かれた柔らかい影、チェッカーボードとブレンドして中間色になっている）まで透明化が伝播するよう、初期の厳しい閾値（diff<=8, brightness>=190）から緩和して調整したもの
2. **孤立断片の除去（後処理）**: フラッドフィル後も、境界から辿り着けない位置に小さなチェッカーボード片が孤立して残ることがあった（コイン下部の柔らかい影の中に、閾値内に収まらない暗めのチェッカー片が島状に取り残されるケース）。そのため、残った不透明ピクセルの連結成分（4近傍）を求め、**最大の連結成分（＝バッジ本体のイラスト）以外は全て透明化**する後処理を追加した
3. 実際の表示サイズ（150px、`.stamp-level-complete-badge-img`のCSS指定サイズ）で目視確認した結果、円形のバッジがきれいに切り抜かれ、縁にごく僅かなギザつきが残る程度で実用上問題ないレベルだった（スクリーンショットをユーザーに送付し確認済み）

この処理を、`badge-central.png`を除く既存9枚全てに適用する。

### 2-2. `badge-central.png`の差し替え

ユーザーから新たに送付されたマーライオンのイラスト（`/home/masahiko/.claude/uploads/cf2492da-07b2-4426-a6e8-f71385083029/5a45b2d8-IMG_2277.png`、1024×1024px）を、他のバッジ画像と同じ256×256pxにリサイズした上で、§2-1と同じチェッカーボード透明化処理を適用し、`public/images/stamp-badges/badge-central.png`を上書きする。プロトタイプ検証では1回のフラッドフィルのみで連結成分が1個（＝後処理での追加除去なし）に収まり、他画像よりもクリーンな結果だった。

## 3. 既存コードの調査結果

### 3-1. `public/app.css`・`public/app.js`

- `.stamp-level-complete-badge-img`（`public/app.css` 2101〜2105行目、設計書84でサイズ拡大済み）: `width/height:150px; object-fit:contain; filter:drop-shadow(...)`。**円形マスク（`border-radius`や`clip-path`）は一切指定されていない**。画像自体が円形イラスト＋透明背景である前提で成立するデザインのため、透明化修正のみでCSS変更は不要
- `.stamp-circle--area-img`・`.stamp-area-badge-img`（エリアバッジ用、設計書80、現状`display:none`で非表示中）・`.stamp-unlock-img`（レベル解禁演出モーダル用、設計書81）も同様に円形マスクなし、画像側の透明化のみに依存する設計。これらも今回の透明化修正の恩恵を受ける（非表示中のエリアバッジも含め、画像ファイル自体を差し替えるため）
- `STAMP_BADGE_AREAS`・`STAMP_LEVEL_META`（`public/app.js`）: 画像パスの参照方法（`meta.img`／`area.img`、いずれも`/images/stamp-badges/badge-*.png`の固定パス文字列）は変更不要。ファイル名・パスとも据え置きで中身のピクセルデータのみ差し替えるため

### 3-2. `server.js`

無関係。画像ファイルは`public/images/`配下の静的アセットであり、`server.js`はこのディレクトリの配信ロジック（Express静的ファイル配信）を一切変更しない。

## 4. スコープ外（今回作らないもの）

- CSS側での円形マスク（`border-radius:50%`や`clip-path:circle()`)の追加。画像自体を透明化する方針を優先し、CSS側の対症療法は行わない（画像自体が既に円形イラストとして描かれているため、透明化さえ直れば円形マスクは不要という設計判断は設計書80から変更なし）
- Nano Bananaでの10枚全量再生成（ユーザーが3択中「自動で透明化」を選んだため、再生成は行わない）
- `badge-central.png`以外の画像内容そのものの変更（East/West/North/North-East/Sentosa/レベル4種は内容は正しいため、透明化処理のみを適用し絵柄自体には触れない）
- 透明化アルゴリズムの汎用スクリプト化・将来のバッジ追加時の自動パイプライン化（今回は10枚への1回限りの適用として実施。将来また同じ問題が起きた場合は本設計書のアルゴリズムを再利用すればよいが、恒久スクリプトとして`scripts/`配下に切り出すことまでは今回のスコープに含めない）

## 5. データモデル・APIの変更点

**変更なし**。`server.js`・`data/`配下・APIレスポンス構造とも無関係、`public/images/stamp-badges/`配下の静的PNGファイル10枚のピクセルデータのみを書き換える。

## 6. 実装詳細（builderへの引き継ぎ事項）

以下のNode.jsスクリプト相当のロジックを実行し、`public/images/stamp-badges/`配下の10ファイルを直接上書きする（一時スクリプトとして`scripts/`外の作業ディレクトリで実行し、処理後は削除して構わない。恒久スクリプトとして残す必要はない）。

```js
// 疑似コード（メインエージェントがプロトタイプ済みのロジック、そのまま流用可）
async function makeTransparent(inputPath, outputPath, { diffTol = 30, minBrightness = 140 } = {}) {
  const { data, info } = await sharp(inputPath).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  const w = info.width, h = info.height, c = info.channels;
  function isCheckerColor(x, y) {
    const i = (y * w + x) * c;
    const r = data[i], g = data[i+1], b = data[i+2];
    const maxC = Math.max(r,g,b), minC = Math.min(r,g,b);
    if (maxC - minC > diffTol) return false;
    if (maxC < minBrightness) return false;
    return true;
  }
  // 1) 境界からのBFSフラッドフィルでチェッカー色ピクセルのアルファを0に
  // 2) 残った不透明ピクセルの連結成分を求め、最大成分以外を透明化
  // (メインエージェントが検証したプロトタイプスクリプトと同一ロジック)
  await sharp(data, { raw: { width: w, height: h, channels: c } }).png().toFile(outputPath);
}
```

**対象ファイルと処理内容**:
1. `badge-central.png`: まず`/home/masahiko/.claude/uploads/cf2492da-07b2-4426-a6e8-f71385083029/5a45b2d8-IMG_2277.png`を`sharp().resize(256,256)`で256×256にリサイズしてから上記透明化処理を適用し、`public/images/stamp-badges/badge-central.png`を上書き
2. `badge-east.png`／`badge-west.png`／`badge-north.png`／`badge-north-east.png`／`badge-sentosa.png`／`badge-level-standard.png`／`badge-level-local.png`／`badge-level-niche.png`／`badge-level-special.png`（計9枚）: 既存ファイルをそのまま入力として上記透明化処理を適用し、同じファイル名で上書き

**確認事項（checker/builderへ）**:
- 処理後、`sharp().ensureAlpha().raw()`でアルファヒストグラムを取得し、10枚とも`alpha=0`のピクセルが一定数存在する（透明化が実際に効いている）ことを確認する
- 各画像を実際にsand色系背景（`#F0E6D3`等、`var(--sand)`相当）に合成した確認画像を150px相当で生成し、チェッカーボードの残骸や不自然な欠けがないか目視確認する
- 画像サイズ（256×256px）・ファイル名・配置ディレクトリは変更しない
- `public/images/`は`.gitignore`対象外（通常のgit管理対象）のため、変更後は必ず`git add`してコミットに含めること

## 7. i18n変更点

**新規キーなし**。画像ファイルの中身のみの変更のため、i18n対応は不要。

## 8. 既知の未解決事項

1. **透明化処理の副作用**: フラッドフィルの閾値（diffTol=30, minBrightness=140）は`badge-level-standard.png`（カメラ）と`badge-central.png`（マーライオン）の2枚でプロトタイプ検証したのみで、残り8枚全てを目視確認したわけではない。閾値が強すぎてイラスト自体の明るいグレー・白系の部分（例: カメラのハイライト、建物の白壁等）を誤って透明化してしまう可能性はゼロではないため、builder実装後に**10枚全てを目視確認すること**を必須とする
2. **iOS実機でのバッジ表示**: 設計書69〜84自体がまだTestFlightビルド未実施のため、本修正も次回一括リリース時にWeb版に続いて確認する
3. **将来バッジを追加する際の再発防止**: Nano Banana等のAI画像生成ツールで「透明背景」を指定しても、実際にはチェッカーボード柄が焼き込まれる場合があるという知見が得られた。今後新規バッジ画像を追加する際は、生成直後に本設計書のアルファヒストグラム確認（`sharp`でアルファ値の分布を見る）を行う習慣を持つことが望ましい（CLAUDE.mdへの教訓記録はcloserに委ねる）

## 9. リスク

1. **既存9枚のうち透明化処理で意図せず絵柄の一部が消えるリスク**: 上記§8-1参照。特に白っぽい配色を含むイラスト（East=建物の白壁、North=鳥の胸元の白い羽毛等）は、フラッドフィルの境界判定次第で絵柄の縁が僅かに侵食される可能性がある。builder実装後の目視確認で発見された場合は、該当画像のみ閾値を個別調整して再処理する
2. **badge-central.png差し替え後の絵柄整合性**: 新しいマーライオン画像は他9枚と異なるプロンプト・生成セッションで作られたものだが、プロトタイプ確認では色味・線画タッチとも既存バッジ群と違和感のない仕上がりだった（ユーザー確認済みのスクリーンショット送付・承認を経て確定）
3. **`.gitignore`対象外であることの再確認**: `public/images/`はdata/とは異なりgit管理対象のため、変更後のコミット漏れがないか`git status`で確認する（CLAUDE.md設計書80の既存注意点の再確認）

## 10. データ共有影響（Web版/iOS App Store版）の確認 ※CLAUDE.md必須項目

1. **後方互換性**: 画像ファイルの中身のみの変更で、APIレスポンス構造・データモデルに変更はない。旧バージョンApp Store版への影響なし（スタンプラリー機能自体が本番未リリースのため実質的な影響対象ユーザーもいない）
2. **影響範囲**: Web版・iOS版の両方に反映される変更（`public/`配下の共通アセットのため）。`server.js`・データファイルは無変更のため`pm2 restart`不要。Web版は静的ファイルのキャッシュのみ考慮（画像ファイル自体は`?v=`パラメータを持たないファイル名固定パスのため、CDN/ブラウザキャッシュが効いている場合は反映に時間がかかる可能性がある点に留意。必要であれば`index.html`・`app.js`側のキャッシュバスティング更新とは別に、画像ファイル名自体へのフィンガープリント付与を検討する余地はあるが、今回はスコープ外とし影響が出た場合は個別対応とする）
3. **iOS版**: Capacitorのローカルバンドル方式のため、次回TestFlightビルドでの反映が必要
...
# 設計書88 — スタンプ一覧カードの見た目刷新（写真サムネイル＋レベル進捗バー＋制覇スタンプ印）

（2026-07-21 メインエージェントがユーザーとの会話・モックアップ2回（v2/v3）のレビューを経て設計。コード実装はorchestratorに依頼する）

## 1. 背景

設計書83で実装したコレクション一覧の状態B（解禁中・未全制覇レベルの横長カード一覧）が、ユーザーから「文字とダッシュ円だけで味気ない」と指摘された。メインエージェントが3案（写真サムネイル追加／レベル進捗バー／「次はここ」カードの強調）を提示し、ユーザーは「1（写真サムネイル）と2（進捗バー）」を選択。モックアップ（v2）をPlaywrightで生成しレビューした後、ユーザーから追加要望「空っぽの◯（バッジ）は消して、代わりに制覇したら『済み』的な画像を表示する」があり、これを反映したv3モックアップも承認された（「いいね👍進めて」）。

## 2. 確定済み仕様（v3モックアップで視覚確認・承認済み）

### 2-1. 写真サムネイル

各`.stamp-card`の左側にある`.stamp-card-circle`（36px、ダッシュ円+番号/チェックマーク）を、`spot.imageUrl`を使った56px角丸正方形の写真サムネイルに置き換える。

- `spot.imageUrl`がある場合: `<img>`で表示（`object-fit:cover`、`border-radius:12px`）
- `spot.imageUrl`が空文字列の場合（設計書73時点で`tekka-market`・`labrador-secret-tunnel`の2件が該当、他は概ね取得済み）: 写真の代わりにプレースホルダー（sand背景の角丸正方形＋📍アイコン）を表示する

### 2-2. 番号バッジの廃止・制覇済みスタンプ印の追加

- **未制覇スポット**: 番号バッジを一切表示しない（写真のみ、またはプレースホルダーのみ）。「次はここ！」タグ（既存、`.stamp-card-next-tag`）は現状維持
- **制覇済みスポット**: 写真の右下角に、レベルカラー（`meta.color`）で塗った円形の「済」スタンプ印（30px、白い縁取り、`rotate(-12deg)`で手押し感を演出）を重ねて表示する。ラベルはi18n対応（ja「済」/en「✓」、絵文字的に短くして円のサイズに収める）

### 2-3. レベルごとの進捗バー

`.stamp-level-section-title`（レベル見出し）の直下に、横長の進捗バー（`checkedCount/totalCount`の割合を塗りつぶし、`var(--caramel)`〜`var(--caramel-dark)`のグラデーション）＋右端に「X/Y」のテキストラベルを追加する。

## 3. 既存コードの調査結果（2026-07-21時点、実ファイル確認済み）

### 3-1. `public/app.js`

- `_renderStampLevelRowInProgress(meta, spotsInLevel, nextTarget, lang)`（3979〜4059行目付近、実際の対象範囲は4023〜4059行目）:
  - `circleHtml`生成部（4030〜4032行目）: `checked`時は`stamp-card-circle stamp-card-circle--checked`+`✓`、未制覇時は`stamp-card-circle`+`spot.order`。**この分岐を全面的に書き換える対象**
  - `.stamp-level-section-title`（4053〜4056行目）: `${meta.emoji} ${t(meta.labelKey)}`のみ。**この直後に進捗バーのHTMLを追加する**
  - 呼び出し元`_renderStampCollectionList()`（3979〜4011行目）で`checkedCount`/`totalCount`は既に算出済み（3991〜3992行目）のため、`_renderStampLevelRowInProgress`の引数に追加して渡す（現状のシグネチャ`(meta, spotsInLevel, nextTarget, lang)`に`checkedCount, totalCount`を追加するか、関数内で`spotsInLevel`から再計算するかはbuilder判断でよい。呼び出し側は3行目〈`_renderStampLevelRowInProgress(meta, spotsInLevel, nextTarget, lang)`〉のみの変更で済む）

### 3-2. `public/app.css`

- `.stamp-card-circle`（2060〜2067行目）・`.stamp-card-circle--checked`（2068〜2070行目）: 他に参照箇所なし（`grep`で確認済み、`public/app.js`内の同一箇所2行のみが参照元）。**安全に削除し、新規クラスに置き換えてよい**
- `.stamp-level-section-title`（2041〜2045行目）: 進捗バー用の新規クラス（例: `.stamp-level-progress-row`/`.stamp-level-progress-track`/`.stamp-level-progress-fill`/`.stamp-level-progress-label`）をこの直後に追加する

### 3-3. データ

- `data/sg/stamp-spots.json`: `imageUrl`フィールドは設計書73で導入済み、23件中21件に実データあり（2件のみ空文字列）。今回のフロントエンド変更のみでデータ変更は不要

## 4. スコープ外（今回作らないもの）

- 状態A（ロック中1行表示）・状態C（全制覇バッジ）の見た目変更（今回はモックアップ通り状態Bのみが対象）
- 制覇済みカード全体の色調・不透明度の変更（v3モックアップでは軽く`opacity`を落とす案も試したが、ユーザーからの明示的な要望ではないため、builder判断で「入れても入れなくても良い」微調整とする。必須要件ではない）
- マップビュー（Leafletピン）・エリアバッジ（非表示中）への写真サムネイル適用
- `tekka-market`・`labrador-secret-tunnel`の画像再取得（既存の未解決事項のまま）

## 5. データモデル・APIの変更点

**変更なし**。`server.js`・`data/`配下とも無関係。`spot.imageUrl`は既存フィールドをそのまま参照するのみ。

## 6. i18n変更点

新規キー1個追加（ja/en同時）:
- `stampCardDoneMark`: ja「済」/ en「✓」（制覇済みスタンプ印のラベル、30px円に収める必要があるため英語は文言ではなくチェックマーク記号にする）

## 7. 既知の未解決事項

1. **英語版のスタンプ印表現**: 「済」は日本語の慣用句的表現で、英語では単純な「✓」に置き換える判断とした。将来的に英語UIでも文字表現にしたい場合は別途検討
2. **画像なしプレースホルダーの実機表示**: 📍アイコン+sand背景のプレースホルダーが、実際の写真付きカードと並んだ時に不自然に見えないかは次回TestFlightビルド後の確認が必要

## 8. リスク

1. **カード高さの変化**: 36px→56pxへのサムネイル拡大により、カード1件あたりの高さが増える。23スポット分のリストのスクロール量が増えるが、視覚的な魅力向上とのトレードオフとしてユーザー承認済み
2. **`.stamp-card-circle`系CSS削除の影響範囲**: §3-2で確認済みの通り他に参照箇所はないため、削除しても安全

## 9. データ共有影響（Web版/iOS App Store版）の確認 ※CLAUDE.md必須項目

1. **後方互換性**: フロントエンドのみの変更、API・データモデルへの影響なし
2. **影響範囲**: `public/`配下の共通コードのため、Web版・iOS版の両方に同時に反映される。`server.js`・データファイルは無変更のため`pm2 restart`不要
3. **iOS版**: 次回TestFlightビルドでの反映が必要

## 承認状況
2026-07-21 メインエージェントがユーザーとの会話・モックアップ2回のレビューを経て設計。ユーザーが最終モックアップ（v3）に対し「いいね👍進めて」と承認。**ユーザー承認済み**。

# 設計書89 — ボトムナビ「制覇」→「探検」・コース画面共通見出し「スタンプラリー」→「シンガポール探訪」への名称変更

（2026-07-21 メインエージェントとユーザーの会話によるネーミングブレストを経て確定。コード実装はorchestratorに依頼する）

## 1. 背景

ユーザーから、ボトムナビ「制覇」（設計書72で「コース」から改名）とコース画面共通見出し「スタンプラリー」（設計書73で「おでかけコース」から改名）の組み合わせが「しっくりこない」との指摘があった。コース画面は「スタンプラリー／みんなのコース／マイコース」の3タブ共通ヘッダーとして常に「スタンプラリー」を表示する設計（設計書73）だが、コース閲覧中も無関係な文言が出続けることが違和感の一因と分析した（この点はCLAUDE.md設計書73自体にも既知の妥協点として記録済み）。

メインエージェントとの会話でネーミング案をブレスト（探検/冒険/攻略/探訪/探究/巡礼等を比較検討）した結果、「シンガポールに住む期間は限られている中で、どれだけ深くシンガポールを知れるか」というユーザーが重視するテーマ性を踏まえ、**ナビは短く衝動性のある言葉、見出しは深みのある言葉、と役割分担する**方針で「ナビ＝探検／見出し＝シンガポール探訪」の組み合わせに決定した。

見出しをタブごとに動的に切り替える案（スタンプラリータブでは「シンガポール探訪」、コースタブでは「みんなのコース」等）も提示したが、ユーザーは**全タブ共通で「シンガポール探訪」に固定する**現状のアーキテクチャ維持を選択した（AskUserQuestionで確認済み）。

## 2. 確定済み仕様

### 2-1. ボトムナビラベルの変更

`navCourse`キーの値のみ変更（キー名は不変、既存パターン踏襲）:
- ja: 「制覇」→「探検」
- en: 「Conquer」→「Explore」

### 2-2. コース画面共通見出しの変更

`courseScreenTitle`キーの値のみ変更（キー名は不変）:
- ja: 「スタンプラリー」→「シンガポール探訪」
- en: 「Stamp Rally」→「Explore Singapore」

### 2-3. 変更対象外（明示）

- `courseTabStampMap`（コース画面内のタブラベル、現在「スタンプラリー」/「Stamp Rally」）: **変更しない**。このキーは3タブのうち1つのタブ自体を指すラベルであり、共通見出し（`courseScreenTitle`）とは別物。スタンプラリー機能そのものの呼称としては引き続き「スタンプラリー」を使う
- `courseTabEveryone`（みんなのコース）・`courseTabMylist`（マイコース）: 変更なし
- コード内の関数名・変数名・CSSクラス名（`stamp*`プレフィックス等）: 変更なし。あくまで表示文言（i18n値）のみの変更

## 3. 既存コードの調査結果

- `public/app.js` 429〜430行目（`STRINGS.ja`）: `navCourse: '制覇',` / `courseScreenTitle: 'スタンプラリー',`
- `public/app.js` 694〜695行目（`STRINGS.en`）: `navCourse: 'Conquer',` / `courseScreenTitle: 'Stamp Rally',`
- `public/index.html` 145行目: `<span class="screen-title" data-i18n="courseScreenTitle">スタンプラリー</span>`（デフォルト直書きテキスト）
- `public/index.html` 514行目: `<span class="nav-label" data-i18n="navCourse">制覇</span>`（デフォルト直書きテキスト）

上記4箇所（STRINGS.ja 2箇所・STRINGS.en 2箇所・index.htmlデフォルト値2箇所、合計6箇所）が変更対象。設計書72・73と同一の「値のみ変更、キー名は不変」パターンを踏襲する。

## 4. スコープ外（今回作らないもの）

- 見出しのタブごとの動的切り替え（§1参照、ユーザーが現状の固定表示維持を選択）
- `courseTabStampMap`等、他のi18nキーの変更
- スタンプラリー機能自体のロジック・データモデルの変更
...
既存キー2個の値変更（ja/en同時、§2-1・2-2参照）。新規キーの追加はなし。

## 7. 既知の未解決事項

1. **見出し固定表示という当初からの違和感の根本解消には至らない**: 名称変更後も「シンガポール探訪」がコース閲覧中も表示され続ける点は変わらない。ユーザーが動的切り替えより固定表示の維持を選んだ結果であり、設計書73時点の妥協点は名称が変わっただけで構造的には残る
2. **英語訳「Explore」「Explore Singapore」の適切性は未検証**（既存の英語訳「Conquer」「Stamp Rally」も設計書72で「将来的な再検討の余地あり」と記録されていた既知の性質と同様）

## 8. リスク

1. **画面遷移的な急な文言変更ユーザー体験**: 既存ユーザー（Web版）が急に文言が変わったことに戸惑う可能性はあるが、スタンプラリー機能自体がまだ本番未リリース（TestFlightビルド未実施）のため実質影響なし

## 9. データ共有影響（Web版/iOS App Store版）の確認 ※CLAUDE.md必須項目

1. **後方互換性**: 表示文言のみの変更、API・データモデルへの影響なし
2. **影響範囲**: `public/`配下の共通コードのため、Web版・iOS版の両方に同時に反映される。`server.js`・データファイルは無変更のため`pm2 restart`不要
3. **iOS版**: 次回TestFlightビルドでの反映が必要

## 承認状況
2026-07-21 メインエージェントとユーザーの会話によるネーミングブレスト（攻略/探検/冒険/探訪/探究/巡礼等の比較検討）を経て、ユーザーが「いいね、探検とシンガポール探訪で行こうか。お願いします。」と承認。見出しの固定/動的表示についてもAskUserQuestionで確認済み（全タブ共通固定を選択）。**ユーザー承認済み**。

# 設計書90 — イベントカード画像の読み込み失敗時、1回だけ自動リトライしてから絵文字フォールバックに切り替える

（2026-07-21 メインエージェントが直接調査。ユーザー報告「Web版だけ画像が表示されずアプリ版は正常」の原因調査結果に基づく設計。コード実装はorchestratorに依頼する）

## 1. 背景

ユーザーから、イベント一覧のあるカード（Thermomix TasteMaster Cook-Off 2026）で、Web版だけ画像の代わりに絵文字（👨‍🍳）の背景が表示され、iOS App版では正常に写真が表示されるとの報告があった。

メインエージェントが調査した結果、当該画像URL（`https://sethlui.com/wp-content/uploads/2026/07/Thermomix-TasteMaster-2026-announcement-featured-image.jpg`）自体は`curl`・Playwrightでの実機再現テストいずれでも現在正常に読み込める（200、実JPEG、1200px幅）ことを確認した。URL自体は壊れていない。

原因は画像URLではなく、**既存の画像エラーハンドリングにリトライ機構が一切ない**ことにあると特定した。

```js
// 現状（public/app.js 1373〜1377行目付近、他2箇所も同型）
const imgHtml = (e.image && ...)
  ? `<img src="${e.image}" ... onerror="handleImgError(this,'${bgClass}','${safeEmoji}')" />`
  : `<div class="card-image-bg ${bgClass}">${e.emoji || '📍'}</div>`;
```
```js
// public/app.js 5432〜5434行目
function handleImgError(el, cls, emoji) {
  el.parentElement.innerHTML = '<div class="card-image-bg ' + cls + '">' + emoji + '</div>';
}
```

`<img>`タグの読み込みが一度でも失敗すると（一時的なネットワークの詰まり、外部ブログ側の瞬間的な不調等）、`onerror`が即座に発火し、親要素のHTML自体を絵文字フォールバックのdivに**恒久的に**置き換えてしまう。リトライは一切行われない。

さらに設計書21（イベントカードのDOM差分更新）により、一度生成したカードのDOM要素は`_cardElCache`でタブ切り替え時も使い回される（都市切替・再フェッチ時のみ`_cardElCache.clear()`でリセットされる）。そのため、Web版のセッション中に一度だけ画像読み込みが瞬断で失敗すると、その後ずっと（ページを再読み込みするまで）絵文字フォールバックのまま固定表示され続けることになる。iOS App版は別セッション・別タイミングでの読み込みだったため、たまたまその瞬断に当たらず正常に表示されたと考えられる。

## 2. 確定済み仕様

### 2-1. `handleImgError()`に1回だけの自動リトライを追加

`public/app.js`の`handleImgError(el, cls, emoji)`（5432〜5434行目）を、初回失敗時は約1.2秒待ってから同じURLで再読み込みを試み、**それでも失敗した場合のみ**絵文字フォールバックに切り替えるよう変更する。
...
function handleImgError(el, cls, emoji) {
  if (!el.dataset.retried) {
    ...
    setTimeout(() => {
      ...
      el.src = src; // src再代入だけでは同一URLの場合ブラウザが再読み込みしないことがあるため、一度removeAttributeで明示的にクリアしてから再設定する
    }, 1200);
    return;
  }
  el.parentElement.innerHTML = '<div class="card-image-bg ' + cls + '">' + emoji + '</div>';
...
- リトライ回数の記録は`el.dataset.retried`（DOM要素自身の属性）で行う。モジュールスコープの状態を持たないため、複数カード間で干渉しない
- リトライが最終的に失敗して絵文字フォールバックに切り替わった場合、`<img>`要素自体が`innerHTML`で破棄されるため、リトライ状態も自然にクリーンアップされる（メモリリークなし）
- `handleImgError`は`public/app.js`内で共有関数として**3箇所から呼ばれている**（メインのイベントカード1378行目・ピン詳細モーダル2545行目・ピン一覧カード2623行目）。**この1つの関数を修正するだけで3箇所すべてに一括で適用される**（個別に3箇所を触る必要はない）

### 2-2. 適用範囲の確認

- `handleImgError`を呼び出す3箇所（メインカード・ピン詳細・ピン一覧）とも、`<img>`タグの`onerror`属性からこの関数を呼ぶ構造は共通のため、関数の修正のみで全て対応できる
- コース詳細画像・マイコースカード画像・スタンプ詳細画像（設計書75で`innerHTML`都度生成方式に統一済み）は、`handleImgError`とは別の独立したエラーハンドリング（`imgEl.onerror = () => { imgEl.style.display = 'none'; }`等）を使っており、**今回の変更の対象外**（構造が異なるため無関係、誤って混同しないよう注意）

## 3. 既存コードの調査結果

- `public/app.js` 1373〜1377行目: メインのイベントカード（`renderEventCard()`）の画像/フォールバック分岐
- `public/app.js` 2542〜2547行目: ピン詳細モーダル（`p.image`使用）の画像/フォールバック分岐
- `public/app.js` 2623行目: ピン一覧カード（`event.image`使用）の画像/フォールバック分岐
- `public/app.js` 5432〜5434行目: `handleImgError(el, cls, emoji)`共有関数
- 画像URL自体の疎通確認（`curl`・Playwright実機再現テスト）: 正常（§1参照）
- Service Worker（`public/sw.js`）・CSP設定・nginx設定: いずれも本件との関連なし（`grep`で確認済み、クロスオリジン画像リクエストを妨げる設定は存在しない）

## 4. スコープ外（今回作らないもの）

- 2回以上の複数回リトライ・指数バックオフ等の高度なリトライ戦略（1回のリトライで十分と判断、過度な複雑化を避ける）
- ロード中のスケルトン表示・プレースホルダーアニメーション等のUX追加演出（既存の「読み込み中は何も出ない」挙動を維持、今回はリトライロジックのみに集中）
- 既にWeb版のブラウザタブで固定表示されてしまっている絵文字フォールバックの自動修復（この修正はコード デプロイ後の新規カード生成・ページ再読み込みにのみ適用される。ユーザーが今すでに開いているタブは、リロードするまで直らない）
- 画像ホスト（`sethlui.com`等の外部ブログ）側の信頼性向上・自前ミラーリング等
...
1. **既に固定表示されてしまっているカードの救済にはならない**（§4参照）。ユーザーには「気になる場合はページを再読み込みしてください」と案内する
2. **1.2秒の待機時間は暫定値**: 実際にどの程度の遅延が「瞬断からの回復」に十分かは未検証。短すぎると再度同じ理由で失敗し、長すぎるとユーザーが違和感を覚える可能性がある。実運用で問題があれば調整する

## 8. リスク

1. **`el.src = src`が実際に再読み込みをトリガーするか**: ブラウザによっては同一URLへの再代入だけでは新規リクエストが発行されない場合があるため、`removeAttribute('src')`で明示的にクリアしてから再設定する実装にする（§2-1のコード参照）。実装後、実際に2回目のネットワークリクエストが発行されることをDevTools/Playwrightで確認する
2. **本当に壊れている画像URL（恒久的な404等）の場合、フォールバック表示までに1.2秒余分にかかる**: 画像が本当に存在しない場合でも同じリトライ処理を通るため、絵文字フォールバックが表示されるまでのタイムラグが増える。ただし視覚的な破綻ではなく単なる遅延のため、許容範囲と判断

## 9. データ共有影響（Web版/iOS App Store版）の確認 ※CLAUDE.md必須項目

1. **後方互換性**: フロントエンドのみの変更、API・データモデルへの影響なし
2. **影響範囲**: `public/`配下の共通コードのため、Web版・iOS版の両方に同時に反映される。`server.js`・データファイルは無変更のため`pm2 restart`不要
3. **iOS版**: 次回TestFlightビルドでの反映が必要。今回の不具合はWeb版での報告だが、iOS版でも理論上同じ瞬断リスクはあるため、両プラットフォームへの適用は妥当

## 承認状況
2026-07-21 メインエージェントが原因調査結果（画像URL自体は正常、リトライ機構の欠如が真因）をユーザーに報告し、ユーザーが「お願いします」と承認。**ユーザー承認済み**。

# 設計書91 — ボトムナビラベルを「探検」→「探訪」に修正（見出しと表記統一）

（2026-07-21 設計書89の直後にユーザーが表記統一を希望、コード実装はorchestratorに依頼する）

## 1. 背景

設計書89でボトムナビ「制覇」→「探検」、コース画面共通見出し「スタンプラリー」→「シンガポール探訪」に変更したが、ユーザーから「ボトムメニューも探訪でいいよ」との追加要望があった。ナビと見出しの言葉が異なる（探検／探訪）よりも、同じ「探訪」で統一した方が一貫性が出るという判断。

## 2. 確定済み仕様

`navCourse`キーの値のみ変更（キー名は不変）:
- ja: 「探検」→「探訪」
- en: `'Explore'`のまま変更なし（既存の英語訳は「探訪」に変えても引き続き自然に対応するため据え置き）

## 3. 既存コードの調査結果

- `public/app.js` 429行目: `navCourse: '探検',` → `'探訪'`に変更
- `public/app.js` 695行目: `navCourse: 'Explore',` （変更なし）
- `public/index.html` 514行目付近: `data-i18n="navCourse"`のデフォルト直書きテキスト「探検」→「探訪」

`courseScreenTitle`（「シンガポール探訪」）・`courseTabStampMap`（「スタンプラリー」）は変更しない。

## 4. スコープ外

他のi18nキー・ロジック・データモデルの変更は一切なし。

## 5〜6. データモデル・API・i18n変更点

**データモデル・API変更なし**。i18nは既存キー`navCourse`の値変更のみ（新規キーなし）。
...
## 承認状況
2026-07-21 ユーザーが「ボトムメニューも探訪でいいよ」と会話内で明示。**ユーザー承認済み**。

# 設計書92 — スタンプラリー地図の見た目改善（セピア調フィルター）＋一覧カードから地図ピンへのフォーカス導線

（2026-07-21 メインエージェントが実際のLeafletマップにCSSフィルターを適用したモックアップ、およびカードデザインのモックアップをそれぞれ視覚確認・ユーザー承認済み。コード実装はorchestratorに依頼する）

## 1. 背景

ユーザーから「地図の見せ方が微妙」との指摘があった。実際の地図画面を確認したところ、OpenStreetMap標準タイルがそのまま表示されており、道路番号や無関係な地名ラベルが多く、アプリ全体のクリーム×キャラメル系の世界観と馴染んでいなかった。また、繁華街エリアでピンが密集して重なる問題もあった。

メインエージェントが「タイルにセピア/暖色系のCSSフィルターをかける」提案を行い、実際のLeafletマップ（本番dosuru.app）にPlaywrightで2種類のCSSフィルター（中程度セピアA案／強めセピアB案）を適用したモックアップを提示。ユーザーはB案（強めセピア、羊皮紙調）を選択した。

続けてユーザーから「一覧のカードと上手く紐付けられる？」との追加要望があり、メインエージェントが「カードに『地図で見る』アイコンを追加し、タップで地図表示に切り替え＋該当ピンへパン&ズーム＋一瞬パルス演出でフォーカスする」案を提示、モックアップ（カードへのアイコン追加）を確認の上、承認された。

## 2. 確定済み仕様

### 2-1. 地図タイルのセピア調フィルター（B案）

Leafletのタイルレイヤーコンテナ（`.leaflet-tile-pane`）に、以下のCSSフィルターを適用する。

```css
.leaflet-tile-pane {
  filter: sepia(0.9) saturate(0.55) hue-rotate(-5deg) brightness(1.12) contrast(0.9);
}
```

このセレクタはLeafletが内部的に生成する要素（タイル画像のみを含むペイン）であり、マーカーアイコン（`.stamp-marker-icon`）やズームコントロール（`.leaflet-control-zoom`）等の他のUI要素には影響しない（Leafletの標準的なペイン分離構造により、タイルペインとマーカーペインは別レイヤーのため）。

### 2-2. 一覧カードから地図ピンへのフォーカス導線

各`.stamp-card`（状態B、解禁中・未全制覇レベルの横長カード一覧）の右端に、新規「地図で見る」アイコンボタン（📍、34px円形、`var(--sand)`背景。「次はここ」カードは`var(--caramel)`背景+白アイコンで強調）を追加する。

タップすると:
1. `_stampViewMode`を`'map'`に切り替え、地図ビューを表示（既存`toggleStampViewMode()`と同様の表示切替）
2. Leafletマップを該当スポットの座標へ`flyTo()`でアニメーション付きパン&ズーム（ズームレベル16程度）
3. パン&ズーム完了後、該当ピンに一瞬だけ広がるリングのパルス演出（既存の「次はここ」ピンの継続的な拡大縮小パルス`stampNextPulse`とは別の、**一度きりで消える**新規アニメーション）を表示

カードのタップ領域（スポット詳細を開く既存の`onclick="openStampSpotDetail(...)"`）とは独立したボタンのため、クリックイベントの伝播を止める（`event.stopPropagation()`）実装が必須。

## 3. 既存コードの調査結果

### 3-1. `public/app.js`

- `toggleStampViewMode()`（3787〜3793行目）・`_applyStampViewMode()`（3795〜3803行目）: マップ⇄一覧の表示切替ロジック。新規`focusStampSpotOnMap(spotId)`関数はこれらと同様のパターンで`_stampViewMode`を`'map'`に設定し`_applyStampViewMode()`を呼ぶ
- `_renderStampMarkers()`（3900〜3922行目）: 現状、生成した`L.marker`インスタンスをどこにも保持していない（`clearLayers()`で都度使い捨て）。**新規にマーカー参照を保持する仕組みが必要**（`spotId → marker`のマップ、例: `_stampMarkerRefs`）
- `_renderStampLevelRowInProgress()`（4023〜4059行目、設計書88で写真サムネイル化済み）: カードHTML生成部に新規「地図で見る」ボタンを追加する対象
- `_ensureStampLeafletMap()`（3869〜3885行目）: `display:none`から復帰した直後は`invalidateSize()`が必要という既存の注意点があり、`focusStampSpotOnMap()`でも同様の考慮が必要（`toggleStampViewMode()`の`setTimeout(() => invalidateSize(), 60)`パターンを踏襲）

### 3-2. `public/app.css`

- `.stamp-marker-icon`（1930〜1937行目）: `transform: rotate(-45deg)`の雫形ピン。内部の`span`は`rotate(45deg)`で打ち消して正立させている
- `.stamp-marker-icon--next`・`@keyframes stampNextPulse`（1940行目・1948〜1951行目）: 既存の「次はここ」ピンの継続的パルス（拡大縮小）。**今回追加する一度きりのフォーカスパルスとは別物**、混同しないこと
- `.stamp-card`系（設計書88で刷新済み）: 新規「地図で見る」ボタン用のCSSを追加する

## 4. スコープ外（今回作らないもの）

- ピンのクラスタリング（Leaflet.markercluster等によるピン密集の解消）。今回はセピアフィルターとカード連携のみ、密集問題への対応は別途検討
- 地図からカードへの逆方向のリンク（ピンタップ時に一覧側の該当カードへスクロール等）。既存通りピンタップは`openStampSpotDetail()`（スポット詳細シート）を開く動作のまま変更しない
- タイル自体を別プロバイダ（Stamen/CARTO等のスタイル済みタイル）に差し替える対応。CSSフィルターのみで完結させる方針（外部サービスへの依存追加・APIキー管理等を避けるため）
...
**新規キーなし**。「地図で見る」ボタンはアイコン（📍）のみで、既存の他のアイコンオンリーボタン（✕閉じるボタン等）と同様のパターンを踏襲するため、テキストラベルは付けない。

## 7. 既知の未解決事項

1. **セピアフィルターの実機での見え方**: Web版のPlaywrightモックアップで確認した色調が、iOS実機（WKWebView、TestFlightビルド後）でも同様に見えるかは未検証
2. **`flyTo()`アニメーション中のパルス演出タイミング**: パン&ズームアニメーションの完了を`setTimeout`の固定時間で待つ実装のため、低スペック端末等でアニメーションが長引いた場合、パルスが早期に発火してズレる可能性がある。Leafletの`moveend`イベントを使えばより正確だが、今回は固定時間待ちのシンプルな実装とし、問題があれば後日調整する

## 8. リスク

1. **`event.stopPropagation()`漏れのリスク**: 新規ボタンをカード内に追加する際、カード全体のクリックハンドラ（`openStampSpotDetail`）との二重発火を防ぐため、ボタンのクリックハンドラで確実に`stopPropagation()`を呼ぶこと。実装後、ボタンタップ時にスポット詳細が誤って開かないことを確認する
2. **セピアフィルターがマーカー・コントロールに影響しないことの確認**: `.leaflet-tile-pane`セレクタのみに絞ることで意図しない影響を避けているが、実装後に実際のLeaflet DOM構造（バージョン1.9.4）でこのセレクタが正しくタイルペインのみを指しているか確認する
...
## 承認状況
2026-07-21 メインエージェントが2回のモックアップ（地図セピアフィルターA/B案、カードへの地図リンクボタン追加）を提示し、ユーザーがそれぞれ「いいねBで」「モックお願いします」→（モック確認後）「お願いします」と承認。**ユーザー承認済み**。

# 設計書93 — カードの「地図で見る」アイコンの色を全カード共通にする（「次はここ」カードのみ強調色だったのを解消）

（2026-07-21 設計書92実装直後、ユーザーが実機スクリーンショットを確認し修正依頼。コード実装はorchestratorに依頼する）

## 1. 背景

設計書92で追加した「地図で見る」アイコンボタン（`.spot-map-link`）は、「次はここ」カードのみ`.spot-map-link--next`修飾子で`var(--caramel)`背景+白アイコンに強調表示する仕様だった。ユーザーが実機スクリーンショットを確認したところ「アイコンの色が一番上（次はここカード）だけ違うので同じにして」と修正依頼があった。ピン自体の色（📍絵文字自体の赤色）は変更不要、あくまでボタンの背景色の差異のみを揃える。

## 2. 確定済み仕様

`.spot-map-link--next`による背景色・文字色の上書きを廃止し、「次はここ」カードかどうかに関わらず全ての「地図で見る」ボタンを同じ見た目（`var(--sand)`背景）にする。

## 3. 既存コードの調査結果

- `public/app.css` 2147〜2159行目:
  ```css
  .spot-map-link {
    flex-shrink: 0;
  ...
  ```
- `public/app.js` 4081行目: `<div class="spot-map-link ${isNext ? 'spot-map-link--next' : ''}" onclick="event.stopPropagation(); focusStampSpotOnMap('${spot.id}')">📍</div>`

## 4. 実装方針

- `public/app.css`の`.spot-map-link--next`ブロック（背景色・文字色の上書き）を削除する
- `public/app.js`側の`${isNext ? 'spot-map-link--next' : ''}`という条件付きクラス付与も、対応するCSSがなくなり無意味になるため合わせて削除してよい（`class="spot-map-link"`のみに簡略化）。**「次はここ」タグ自体（`.spot-card-next-tag`、既存のオレンジのタグ表示）は変更しない**、あくまで地図アイコンボタンの色だけの話

## 5. スコープ外

- 「次はここ」タグ・カード全体の強調表示（ボーダー色等）は変更しない
- 📍絵文字自体の色は変更しない（絵文字自体の配色はOS依存でCSSで変更不可）

## 6. データ共有影響（Web版/iOS App Store版）の確認

CSS/JSのみの軽微な変更。`public/`配下の共通コードのためWeb版・iOS版両方に反映。`server.js`・データファイル無変更のため`pm2 restart`不要。iOS版は次回TestFlightビルドで反映。

## 承認状況
2026-07-21 ユーザーが実機スクリーンショットを確認し「アイコンの色が一番上だけ違うので同じにして。色を変えなくていいです」と明示。**ユーザー承認済み**。

# 設計書94 — マイコースタブの非表示化＋コース作成FABを地図/一覧切り替えボタンに転用

（2026-07-21 ユーザーとの会話を経て確定。「地図で見るボタンの置き場所を変えたい」という相談から、マイコース機能自体の非表示化・既存FABの転用という統合案に発展。コード実装はorchestratorに依頼する）

## 1. 背景

ユーザーから「地図で見る／一覧を見る」ボタン（`#stamp-view-toggle-btn`、現在タブ下に単独で浮いている）の見せ方を変えたいとの相談があった。メインエージェントが「画面右下のFAB化」を提案したところ、ユーザーから「コース作成FABと衝突しないか」という指摘があり、そこから「マイコース機能自体（まだ誰にも使われていない）をタブごと非表示にし、浮いていたコース作成FABを地図/一覧切り替えボタンとして転用する」という統合案に至った。

## 2. 確定済み仕様

### 2-1. マイコースタブの非表示化

`public/index.html`のコース画面タブバー（`.course-tab-bar`）内、`data-tab="mylist"`の「マイコース」ボタンを`display:none`で非表示にする。**削除ではなく非表示**（このプロジェクトの既存パターン「機能は残しつつ表示だけ止める」を踏襲、`switchCourseTab('mylist')`のロジック・`data-i18n="courseTabMylist"`等は一切削除しない）。

### 2-2. コース作成FAB（`#course-fab`）を地図/一覧切り替えボタンに転用

- `onclick`を`openCourseSheet()`から`toggleStampViewMode()`に変更
- アイコンを固定の「＋」から、現在の表示モードに応じた動的アイコンに変更（既存i18nキー`stampViewToggleMap`（🗺️）/`stampViewToggleList`（📖）の絵文字部分をそのまま流用: リスト表示中は🗺️（タップで地図へ）、地図表示中は📖（タップで一覧へ））
- **表示/非表示の条件を反転**: 現状`switchCourseTab()`内で「スタンプラリータブでは`course-fab`を隠し、それ以外のタブで表示する」実装だったが（コース作成はスタンプラリータブに無関係だったため）、転用後は逆に「スタンプラリータブでのみ表示し、みんなのコースタブでは隠す」に変更する（地図/一覧切り替えはスタンプラリータブでのみ意味を持つため）
- タッチデバイス向けの`touchend`デリゲーション登録（`public/app.js`、`{ id: 'course-fab', fn: () => openCourseSheet() }`）も、`onclick`と同様に`fn: () => toggleStampViewMode()`へ変更する（onclick属性とtouchendハンドラの対応関係を崩さないため必須）

### 2-3. 旧・上部の切り替えボタンの非表示化

`#stamp-view-toggle-btn`（現在タブ下に単独で表示されているピル型ボタン）は、機能がFABに移るため`display:none`で非表示にする（削除ではない。`toggleStampViewMode()`本体・関連ロジックは変更なし）。

## 3. 既存コードの調査結果

- `public/index.html` 150行目: `<button class="course-tab" data-tab="mylist" onclick="switchCourseTab('mylist')" data-i18n="courseTabMylist">マイコース</button>`
- `public/index.html` 168〜170行目: `#stamp-view-toggle-btn`を含む行（`<div style="padding:14px 16px 6px;display:flex;align-items:center;justify-content:flex-end;gap:8px;">`）
- `public/index.html` 186〜187行目: `#course-fab`（`class="fab-plan"`、現状「＋」固定テキスト、`onclick="if(!_touchCapableDetected) openCourseSheet()"`）
- `public/app.js` `switchCourseTab()`（3626〜3670行目）: 3637〜3646行目で`tab==='map'`時に`courseFabEl.style.display='none'`、それ以外で`courseFabEl.style.display=''`としている。**この条件を反転させる**
- `public/app.js` 2079行目: `{ id: 'course-fab', fn: () => openCourseSheet() }`（touchendデリゲーション登録、`fn`を変更する対象）
- `public/app.js` `_applyStampViewMode()`（3795〜3803行目）: 現状`#stamp-view-toggle-btn`の`textContent`を更新している。**この関数に`#course-fab`のアイコン更新処理を追加する**（`#stamp-view-toggle-btn`側の更新処理は、対象要素が非表示になるだけで実害はないため残置してよい）
- `public/app.js` `openCourseSheetFromEvent()`（4733〜4736行目）: イベントカードの「🗺 コース作成」ボタンから呼ばれる別の入口で、内部で`openCourseSheet()`を呼ぶ。**この入口は今回のスコープ外**（§4参照）
- `public/app.css` `.fab-plan`（1740〜1755行目）: 52px円形、font-size 22px、中央寄せ。絵文字アイコンでも問題なく収まるサイズ

## 4. スコープ外・既知の残課題（明示）

- **イベントカードの「🗺 コース作成」ボタン（`openCourseSheetFromEvent()`）は今回変更しない**。マイコースタブを非表示にした後もこのボタンは動作し、生成されたコースは引き続き`localStorage`の`{city}_my_courses`に保存されるが、閲覧するタブが無いため事実上「作っても見れない」状態になる。ユーザーからは今回このボタンへの言及がなかったため対応しないが、次回同様の指摘があれば別途対応する
- マイコースのデータ（既存の`localStorage`保存内容、`community-courses.json`への公開有無等）は一切変更・削除しない。あくまで画面上の入口を隠すのみ
- `COURSE_TABS`定数（CLAUDE.md既知の通り元々未使用と判明済み）への追加対応は不要
...
**新規キーなし**。既存の`stampViewToggleMap`/`stampViewToggleList`の絵文字部分を流用。

## 7. リスク

1. **`touchend`デリゲーションの更新漏れ**: `#course-fab`のonclick変更と`touchend`ハンドラ変更は必ずセットで行うこと（CLAUDE.md「onclick属性＋touchendハンドラの二重登録」既知パターンの逆側、片方だけ変更すると挙動不一致になる）
2. **`switchCourseTab()`の表示条件反転ミス**: 現状「map以外で表示」だったのを「mapで表示」に反転する際、みんなのコースタブで誤って残存表示されないか実装後に確認する

## 8. データ共有影響（Web版/iOS App Store版）の確認

フロントエンドのみの変更。`server.js`・データファイル無変更のため`pm2 restart`不要。Web版・iOS版両方に反映、iOS版は次回TestFlightビルドで反映。

## 承認状況
2026-07-21 ユーザーとの会話で確定。「マイコースタブごと非表示にする」「FABは地図の表示に変えます」と明示。**ユーザー承認済み**。

# 設計書95 — イベントカードの「コース作成」ボタンを非表示化（残り2ボタンは現サイズ維持）

（2026-07-21 設計書94の残課題として指摘、ユーザーがサイズ指定つきで即承認。コード実装はorchestratorに依頼する）

## 1. 背景

設計書94でマイコースタブを非表示にした際、メインエージェントから「イベントカードの『🗺 コース作成』ボタンからも引き続きコースが作成できてしまい、閲覧するタブが無いため実質使えない状態が残る」と残課題を報告した。ユーザーから「気になります。こちらも非表示にして」と明示の指示があり、あわせて「等幅を使わず、予定表に登録・ピン留めのボタンを今と同じサイズにして残してください」という制約が示された。

## 2. 確定済み仕様

### 2-1. イベントカードの「コース作成」ボタンを削除

`renderEventCard()`（`public/app.js` 1392〜1404行目）の`.card-action-row`内、3つ目のボタン（🗺 コース作成、`onclick="openCourseSheetFromEvent('${e.id}')"`）を削除する。`openCourseSheetFromEvent()`関数自体は削除しない（他の呼び出し元が残るため、§4参照）。

### 2-2. 残る2ボタン（📌ピン留め／📅予定に追加）は現在の見た目サイズを維持

`.card-action-btn`は`flex:1`（等幅で行いっぱいに伸びる）がベースのCSSクラスだが、これは`.card-action-row`（3ボタン）や他の画面（`#schedule-plan-action-add-btn`等の2ボタン行）でも共有されているため、**共有クラス自体は変更しない**。

代わりに、このコードベースには既に同じ課題への対応パターンが存在する（`public/app.js` 4662行目・6399行目・6403行目、いずれもマイコースカード等で使われている）:
```html
<button class="card-action-btn" style="flex:none;width:calc(33% - 4px);" ...>
```
これと同じインラインstyle（`flex:none;width:calc(33% - 4px);`）を、イベントカードの残る2ボタン（ピン留め・予定に追加）にも追加する。これにより、3ボタン分の行の中で元々占めていた約1/3幅のサイズ感をそのまま維持し、3つ目のボタンが消えた分の余白は右側の空きスペースとして残る（残り2ボタンが引き伸ばされて等幅で埋まることはない）。

## 3. 既存コードの調査結果

- `public/app.js` 1392〜1404行目: `renderEventCard()`内の`.card-action-row`（該当箇所）
- `public/app.css` 513〜539行目: `.card-action-row`（`display:flex;gap:6px;...`）・`.card-action-btn`（`flex:1;...`、共有クラスのため変更しない）
- `public/app.js` 4662行目・6399〜6403行目: 既存の「`flex:none;width:calc(33% - 4px);`」インラインstyle使用例（マイコースカード等）。今回はこの確立済みパターンをそのまま踏襲する
- `openCourseSheetFromEvent(eventId)`関数（`public/app.js` 4735行目）の呼び出し元は3箇所存在する:
  1. `public/app.js` 1401行目: イベントカード（**今回削除する対象**）
  2. `public/app.js` 2565行目: ピン詳細モーダル（`.sale-pin-btn`、別UI）
  3. `public/app.js` 6057行目: 予定詳細画面（`.plan-to-plan-btn`、別UI）

## 4. スコープ外（今回作らないもの、次回検討事項として明示）

**ピン詳細モーダル（2565行目）・予定詳細画面（6057行目）の「コース作成」ボタンは今回のスコープ外**。ユーザーの指示はイベントカード（ピン留め・予定に追加ボタンと並ぶ3ボタン行）に限定されていたため、他の2箇所は変更しない。ただしこれらも同様に「作成はできるが閲覧するタブが無い」という同種の問題を抱えたまま残る。次回ユーザーから同様の指摘があれば別途対応する。

`openCourseSheetFromEvent()`関数自体・`openCourseSheet()`関数自体は削除しない（他の呼び出し元が現役のため）。
...
**新規キーなし**。既存の`pinBtn`/`pinnedBtn`/`addToPlanBtnShort`はそのまま使用。`courseCreateBtnShort`キー自体はこの箇所での参照が無くなるが、他画面で使われていないか確認した上で、使われていなければ死にキーとして残置してよい（削除は必須ではない）。

## 7. リスク

1. **他画面での`.card-action-btn`共有クラスへの影響なし**の確認: インラインstyle上書きのみで対応するため、共有クラスを使う他画面（`#schedule-plan-action-add-btn`等）への影響はゼロのはずだが、実装後に念のため確認する
...
## 承認状況
2026-07-21 ユーザーが「気になります。こちらも非表示にして。なお等幅を使わず、予定表に登録、ピン留めのボタンを今と同じサイズにして残してください。」と明示。**ユーザー承認済み**。

# 設計書96 — ボトムナビ切り替え時にスタンプ関連モーダルが閉じ残るバグの修正

（2026-07-21 ユーザー報告。コード実装はorchestratorに依頼する）

## 1. 背景

ユーザーから、探訪（スタンプラリー）タブでスポット詳細モーダル（`#stamp-spot-detail-sheet`）を開いた状態のまま、ボトムナビをタップして他画面（イベント／予定表／設定）に切り替えられてしまい、モーダルが開いたまま残ることが報告された。

原因を調査したところ、`switchNav(screen)`（`public/app.js` 3510行目）が画面遷移時に`closeAllPopups()`（3491〜3504行目）を呼び、既存の主要モーダル（カレンダーポップアップ・ピン詳細・ピンピッカー・絵文字ピッカー・予定アクションシート・イベント絞り込みシート・コース詳細・コース作成シート・日付ピッカー・予定モーダル）を一括で閉じる仕組みになっているが、**スタンプラリー機能（設計書69・70）のモーダル2つ（`closeStampSpotDetail()`・`closeStampLevelUnlockModal()`）がこのリストに含まれていない**ことが判明した。スタンプラリー機能が`closeAllPopups()`の実装後に追加された際、この関数への追加が漏れていたと考えられる。

## 2. 確定済み仕様

`closeAllPopups()`（`public/app.js` 3491〜3504行目）に、以下2つの既存クローズ関数の呼び出しを追加する。

- `closeStampSpotDetail()`（スポット詳細シート、設計書69）: ユーザーが直接報告した不具合の対象
- `closeStampLevelUnlockModal()`（レベル解禁演出モーダル、設計書70）: 同じ「`closeAllPopups()`未登録」という原因構造を持つ同種の見落としのため、再発防止のためあわせて対応する

## 3. 既存コードの調査結果

- `public/app.js` 3491〜3504行目: `closeAllPopups()`の全文（既存10関数+detail-screenの手動非表示）
- `public/app.js` 3510〜3516行目: `switchNav()`冒頭、フォーカス解除の直後に`closeAllPopups()`を呼ぶ構造
- `public/app.js` 4164行目: `closeStampSpotDetail()`定義（オーバーレイ・シートの`.visible`クラス除去、`_stampSelectedSpot = null`等）
- `public/app.js` 4313行目: `closeStampLevelUnlockModal()`定義
- いずれの関数も、モーダルが**既に閉じている状態で呼んでも安全**（`classList.remove('visible')`は既に付いていなくてもエラーにならない）ため、`closeAllPopups()`から無条件に呼び出しても副作用はない

## 4. スコープ外

- `closeAllPopups()`に登録されている他の関数の見直し・整理は行わない
- スタンプラリー機能以外のモーダルで同様の見落としがないかの全数監査は今回のスコープ外（今回の2件のみ対応）
...
## 承認状況
2026-07-21 ユーザーが「探訪でスポット情報を表示した状態でボトムメニューを切り替えることができます。ボトムメニューを切り替えたらスポットのモーダルは消えるようにして。」と明示。**ユーザー承認済み**。

# 設計書97 — 「地図で見る」ボタンをカードからスポット詳細モーダルへ移動

（2026-07-21 ユーザーの方針転換により確定。コード実装はorchestratorに依頼する）

## 1. 背景

設計書92・93で一覧カード（`.stamp-card`）の右端に追加した「地図で見る」アイコンボタン（`.spot-map-link`）について、ユーザーから「カードじゃなくて、スポットを表示したときのモーダルの中にしようかな」と方針転換の申し出があった。カード一覧の右端という場所よりも、スポット詳細モーダル（`#stamp-spot-detail-sheet`）内に配置する方が自然という判断。

## 2. 確定済み仕様

### 2-1. カードから「地図で見る」ボタンを削除

`_renderStampLevelRowInProgress()`（`public/app.js`）が生成する`.stamp-card`から、設計書92・93で追加した`.spot-map-link`ボタンを削除する（設計書88時点のカード構成に戻す）。

### 2-2. スポット詳細モーダルに「地図で見る」ボタンを追加

`#stamp-spot-detail-sheet`（`public/index.html` 802〜821行目）内、`#stamp-spot-detail-area`（エリア表示）の直後に、小さなテキストリンク形式の「📍 地図で見る」ボタンを追加する。既存の`.card-detail-link`（`public/app.css` 505〜512行目、13px・`var(--caramel)`色・下線なし）と同様の見た目の控えめなリンクスタイルにする。チェックインボタン（プライマリCTA）と競合しない、副次的なアクションとしての位置づけ。

タップ時の処理:
1. `focusStampSpotOnMap(_stampSelectedSpot.id)`を呼ぶ（設計書92で実装済みの既存関数、そのまま再利用）
2. `_stampSelectedSpot`は`openStampSpotDetail()`内で既に設定済みのモジュールスコープ変数のため、`doStampCheckin()`ボタン等と同じパターンで参照できる（引数なしで呼び出し可能、`onclick="if(!_touchCapableDetected) focusStampSpotOnMap(_stampSelectedSpot && _stampSelectedSpot.id)"`のような形）

### 2-3. `focusStampSpotOnMap()`の修正: モーダルを閉じてから地図へ切り替える

設計書92時点の`focusStampSpotOnMap(spotId)`は、一覧カードから呼ばれる前提で「モーダルを閉じる」処理を含んでいなかった。今回、スポット詳細モーダル自身から呼ばれるケースが追加されるため、関数の冒頭に`closeStampSpotDetail();`を追加し、地図へ切り替える前に確実にモーダルを閉じるようにする。
...
`closeStampSpotDetail()`はモーダルが既に閉じている状態で呼んでも安全（設計書96で確認済みの性質と同じ）なため、一覧カードのような呼び出し元が今後増えても問題ない。

## 3. 既存コードの調査結果

- `public/app.js` `_renderStampLevelRowInProgress()`: 設計書92・93で追加された`.spot-map-link`ボタン生成部（削除対象）
- `public/index.html` 802〜821行目: `#stamp-spot-detail-sheet`の全体構造。812行目`#stamp-spot-detail-area`の直後が追加位置
- `public/app.js` `focusStampSpotOnMap(spotId)`（設計書92で新規実装済み）: 冒頭に`closeStampSpotDetail();`を追加する対象
- `public/app.js` `openStampSpotDetail()`: `_stampSelectedSpot = spot;`を設定する箇所（モーダル内ボタンが参照する変数）
- `public/app.css` `.card-detail-link`（505〜512行目）: 見た目の参考にする既存スタイル

## 4. スコープ外

- 設計書92の地図セピアフィルター・パルス演出自体は変更しない
- 設計書93（アイコン色統一）は、対象ボタン自体が今回削除されるため実質的に無意味になるが、コード上の巻き戻し作業は不要（`.spot-map-link`ごと削除するため）
...
**新規キーなし**（「📍 地図で見る」はi18n非対応の固定絵文字+テキストとするか、既存の`stampViewToggleMap`の文言を流用するかはbuilder判断。新規キー追加は必須ではない）。
...
# 設計書103 — レベルラベルの文言再修正（「移住したて」→「新参者」、「定住」→「定住レベル」）

（2026-07-21 設計書98実装直後、ユーザーが「移住」は駐在員には合わないと再検討。コード実装はorchestratorに依頼する）

## 1. 背景

設計書98で確定・実装した「移住したて」について、ユーザーから「駐在の場合は移住とはいわない」との指摘があった。代替案（来たばかり／シンガポール初心者／新参者）を提示し、ユーザーが「新参者」を選択。あわせて「定住」も「定住レベル」に変更したいとの要望があった。

## 2. 確定済み仕様

既存キーの値のみ再変更（設計書89・91・98と同じパターン）:

| キー | 設計書98時点の値（ja） | 今回の新しい値（ja） | en |
|---|---|---|---|
| `stampLevelStandard` | 移住したて | 新参者 | 変更なし（Newcomer） |
| `stampLevelLocal` | 定住 | 定住レベル | 変更なし（Settled） |

`stampLevelNiche`（シンガポール通/Singapore Expert）・`stampLevelSpecial`は変更しない。英語値は日本語の「レベル」付与に合わせた直訳はせず、既存の自然な英単語（Newcomer/Settled）のまま据え置く（設計書98の判断を踏襲、逐語訳にこだわらない既存方針）。

## 3. 既存コードの調査結果

`public/app.js`の`STRINGS.ja`内、`stampLevelStandard`・`stampLevelLocal`（設計書98で値変更済みの箇所、現在の値は「新参者」「定住」ではなく「移住したて」「定住」）。`public/index.html`にデフォルト直書きテキストがあれば同様に確認・変更する。

## 4. スコープ外

`stampLevelNiche`・`stampLevelSpecial`・英語値・その他のロジックは変更しない。

## 5〜7. データモデル・API・i18n・データ共有影響

**変更なし**（表示文言のみ）。`server.js`・データファイル無変更のため`pm2 restart`不要。Web版・iOS版両方に反映、iOS版は次回TestFlightビルドで反映。

## 承認状況
2026-07-21 ユーザーが「新参者、かな、後は定住、ではなく定住レベルとかはどう？」と明示。**ユーザー承認済み**。

# 設計書104 — スペシャルレベルのラベルを「極めし者」に変更

（2026-07-21 ユーザー発案。コード実装はorchestratorに依頼する）

## 1. 背景

設計書98・103でレベルラベルを「新参者→定住レベル→シンガポール通」と在住歴ベースの命名に統一した流れで、ユーザーから最上位の`special`（スペシャル）レベルについても「極めし者」という案が出た。新参者〜シンガポール通までの3段階が在住歴ベースの落ち着いたトーンなのに対し、最後だけ少しドラマチックな響きになることで、隠し要素（該当2スポットのみ、条件も厳しい`special`レベル）にふさわしい特別感を演出できるという狙い。

## 2. 確定済み仕様

`stampLevelSpecial`キーの値のみ変更（キー名は不変）:
- ja: 「スペシャル」→「極めし者」
- en: 「Special」→「Grandmaster」

## 3. 既存コードの調査結果

`public/app.js`の`STRINGS.ja`/`STRINGS.en`内`stampLevelSpecial`（設計書98・103では変更対象外だった箇所）。`public/index.html`にデフォルト直書きテキストがあれば同様に確認・変更する。設計書100で追加した「🔒 スペシャル ？？？」表示（`_renderStampLevelRowLocked()`経由、`t(meta.labelKey)`を参照）も自動的に「🔒 極めし者 ？？？」に切り替わる（ロジック変更不要、i18nキーの値変更のみで反映される）。

## 4. スコープ外

`STAMP_LEVEL_GATES`・解禁条件ロジック・データモデルは変更しない。
...
# 設計書109 — 全制覇バッジを横並びレイアウトに変更＋地図/一覧切り替えFABのアイコンを記号に変更
...
### 1-1. 全制覇バッジの横並びレイアウト
設計書108で実装した全制覇バッジ（縦積み中央寄せ、バッジ画像150px→タイトル→件数→トグルボタン）について、ユーザーから「スタンプ（バッジ画像）は左、『新参者 制覇！』は右、という横並びにしたい」との要望があった。当初76pxへの縮小案も検討したが、ユーザーから「スタンプのサイズは大きいほうがいいので変えないで」と明示があり、**150pxのまま左に配置し、テキスト（タイトル・件数・トグルボタン）を右に並べる横長バナー型**で確定した。モックアップで確認済み。

### 1-2. 地図/一覧切り替えFABのアイコン変更
設計書94でコース作成FAB（「＋」）を地図/一覧切り替えボタンに転用した際、アイコンを絵文字（🗺️/📖、表示モードに応じて動的切替）にしていたが、ユーザーから「地図と本の絵文字がちょっと浮いている（＋ボタンの時と統一感がない）」との指摘があった。「＋」と同じ単色記号のトーンに合わせ、**⇄（左右反転の矢印記号、状態に関わらず固定）**に変更することで確定した。

## 2. 確定済み仕様

### 2-1. 全制覇バッジの横並びレイアウト

`.stamp-level-complete-badge`を`flex-direction:column;align-items:center;text-align:center`から`display:flex;align-items:center;gap:16px;text-align:left`（横並び）に変更する。

- バッジ画像（`.stamp-level-complete-badge-img`）: **150px×150pxのまま変更しない**、`flex-shrink:0`を追加して左側に固定配置
- タイトル・件数・トグルボタンをまとめる新規ラッパー要素（`.stamp-level-complete-badge-body`、`flex:1;min-width:0`）を追加し、右側に配置
- padding: 縦積み時の`32px 20px`から、横並びに合うよう `16px 18px`程度に調整（builder判断で微調整可、モックアップ参照）

### 2-2. FABアイコンを⇄に変更

`_applyStampViewMode()`（`public/app.js`）の以下の行を変更する:
...
状態に応じた出し分けが不要になるため、単純な固定文字列に変更する。`public/index.html`側の`#course-fab`初期テキスト（設計書94で設定した初期値）も同様に`⇄`に変更する。

## 3. 既存コードの調査結果

- `public/app.css` `.stamp-level-complete-badge`系（設計書83〜84・108で拡張済み）: 全面的にレイアウト変更
- `public/app.js` `_applyStampViewMode()`（3802〜3812行目付近）: 3811行目が変更対象
- `public/index.html` `#course-fab`: 初期テキスト値の確認・変更

## 4. スコープ外

- FABの表示/非表示ロジック（スタンプラリータブのみ表示、設計書94で確立済み）は変更しない
- 全制覇バッジの開閉トグル機能・コンパクトカード一覧自体のロジック（設計書108）は変更しない、レイアウトのみの変更

## 5〜7. データモデル・API・i18n・データ共有影響

**変更なし**（レイアウト・アイコンのみの変更）。i18n新規キーなし。`server.js`・データファイル無変更のため`pm2 restart`不要。Web版・iOS版両方に反映、iOS版は次回TestFlightビルドで反映。
...
# 設計書126 — 来星日の✕リセットボタンを削除

（2026-07-23 ユーザーが実機スクリーンショットで「要らない」と判断。コード実装はorchestratorに依頼する）
...
`public/index.html`から`#arrival-date-reset-btn`要素を削除。`public/app.js`から新規関数`_resetArrivalDate()`、`initSettingsProfile()`・`_saveArrivalDate()`内のリセットボタン表示切り替え処理、設定画面touchendデリゲーション一覧内の`#arrival-date-reset-btn`判定行を削除する。

来星日の表示（ピルボタン、設計書125）・透明化した`<input type="date">`・「未設定」表示ロジック（設計書123）は無変更のまま維持する。

## 3. 既存コードの調査結果

- `public/index.html`: 設計書124で追加した`#arrival-date-reset-btn`
- `public/app.js`: `_resetArrivalDate()`（設計書124で追加）、`initSettingsProfile()`・`_saveArrivalDate()`内のリセットボタン表示切り替え処理（設計書124で追加）、設定画面touchendデリゲーション一覧内の該当行（設計書124で追加）

## 4. スコープ外

ピルボタンのスタイル（設計書125）・「未設定」表示ロジック（設計書123）は無変更。
...
# 設計書134 — コレクション一覧カードのメモ・説明文プレビューを削除、チェックイン日付のみ表示

（2026-07-23 ユーザーがモック3案から「日付だけ」を選択。コード実装はorchestratorに依頼する）

## 1. 背景

design 121で訪問済みカードにメモ全文（無ければスポット説明文）をプレビュー表示するようにしたところ、カードごとに高さがバラつく問題が発覚。モック3案（現状=全文／案1=メモ1行省略／案2=済マークのみ）を提示したところ、ユーザーは「チェックイン日付だけ出そうかな」という第4の折衷案を選択した。写真サムネイル（56px、個人の思い出写真があればそちら優先）自体は一覧に残したまま、日付のテキストのみ添える。メモ全文・詳細な情報は既存の「あなたの記録」詳細シート（ポラロイド演出）に一本化する。

なお、全制覇バッジの展開カード一覧（`_renderStampLevelRowComplete()`、design 108）は元々チェックイン日付のみを表示する実装（メモ・説明文プレビューは無し）だったため、今回の変更で状態B（未全制覇時の横長カード）・状態C（全制覇時の展開カード）の表示ロジックが統一される。

## 2. 確定済み仕様

`_renderStampLevelRowInProgress()`（`public/app.js`）内のカード生成部分から、メモ・スポット説明文のプレビュー行を削除し、チェックイン日付のみを残す:
...
サムネイル部分（個人の思い出写真を優先して使うロジック、`memoryPhotoUrl`/`thumbSrc`）は無変更のまま維持する。

## 3. 既存コードの調査結果

- `public/app.js` `_renderStampLevelRowInProgress()`: 上記の変更対象
- `public/app.js` `_renderStampLevelRowComplete()`: 元々チェックイン日付のみの表示のため無変更（参考・整合性確認済み）
- `.stamp-card-desc`クラス（`public/app.css`）: 参照元がこの変更で無くなるが、他箇所（`.stamp-complete-card`系）からは使われていないため死にクラス化する。実害がないため削除はしない（既存の「使わなくなった導線は残置」方針を踏襲）

## 4. スコープ外

スポット詳細シート「あなたの記録」セクション（design 121・127）は無変更（メモ全文・ポラロイド写真の表示はそのまま）。
...
# 設計書138 — 思い出写真ピッカーの診断ログを削除（design 131の後始末）

（2026-07-24 ユーザーがカメラ/フォトライブラリ選択の実機動作を確認済み。コード実装はorchestratorに依頼する）

## 1. 背景

design 131で`_pickStampMemoryPhotoBlob()`に追加した診断ログ（`stamp_memory_photo_pick_start`/`_result`/`_error`）により、design 137でカメラピッカー起動不能の根本原因（`NSPhotoLibraryAddUsageDescription`欠落）を特定・修正できた。ユーザーが実機でカメラ/フォトライブラリ選択の正常動作を確認済みのため、CLAUDE.md既定の運用ルール通り、役目を終えた使い捨て診断ログを削除する。

## 2. 確定済み仕様

`public/app.js`の`_pickStampMemoryPhotoBlob()`から、design 131で追加した3箇所の`_sendDebugLog()`呼び出し（`stamp_memory_photo_pick_start`/`stamp_memory_photo_pick_result`/`stamp_memory_photo_pick_error`）を削除する。関数本体のロジック（プラグイン取得・`getPhoto()`呼び出し・Web版`<input type=file>`分岐）は無変更のまま維持する。

`_sendDebugLog`関数自体・`POST /api/debug-log`エンドポイント自体は恒久ユーティリティのため削除しない（他機能の計装で引き続き使われる）。

## 3. 既存コードの調査結果

- `public/app.js` `_pickStampMemoryPhotoBlob()`（design 131で診断ログ追加、design 137で根本原因特定・解決済み）

## 4. スコープ外

記念日通知本体・思い出機能本体のロジックは無変更。

## 5〜7. データモデル・API・データ共有影響

**変更なし**。`server.js`・データファイル無変更のため`pm2 restart`不要。キャッシュバスティングを更新。Web版・iOS版両方に反映。

## 承認状況
2026-07-24 ユーザーが「ダミーのスポット消して大丈夫です」＋カメラ動作確認済みの流れを受け、CLAUDE.md既定の使い捨てログ削除ルールに基づき実施。

# 設計書139 — 探訪ティアの年数目安ラベルを変更（見習い0〜1年、定住レベル2〜4年）

（2026-07-24 ユーザーが年数目安の見直しを要望。コード実装はorchestratorに依頼する）

## 1. 背景

ユーザーから探訪ティアの年数目安表示を「見習いが0〜1年、定住が2年〜4年、（シンガポール通は）5年以上、（極めし者は）10年以上」に変更したいとの要望。シンガポール通・極めし者は現状のまま変更なし。

## 2. 確定済み仕様

`public/app.js`の`STAMP_LEVEL_META`（3784-3785行目）の`yearRange`/`yearRangeEn`の値のみ変更（キー名・構造は不変）:

```js
// 変更前
standard: { ..., yearRange: '1〜2年', yearRangeEn: '1-2 years' },
local:    { ..., yearRange: '3〜4年', yearRangeEn: '3-4 years' },
// 変更後
standard: { ..., yearRange: '0〜1年', yearRangeEn: '0-1 years' },
local:    { ..., yearRange: '2〜4年', yearRangeEn: '2-4 years' },
```

`niche`（5年以上/5+ years）・`special`（10年以上/10+ years）は無変更。

## 3. 既存コードの調査結果

- `public/app.js` 3784-3787行目: `STAMP_LEVEL_META`定数
- `_stampLevelYearRange(meta)`（3790-3794行目付近）: 値を参照するのみで無変更

## 4. スコープ外

レベル名自体（見習い/定住レベル/シンガポール通/極めし者）・絵文字・色・画像は無変更。
...
# 設計書164 — シェアカードのデザインを「卒業アルバムの1ページ風」に統一（3種類のカード共通スタイル）

（2026-07-25〜26 design 163実装後、ユーザーがカードデザインに「なんか違う」とフィードバック。複数ラウンドのモック提示を経て「卒業アルバムの1ページ風（リボン＋手書き風メッセージ）」に確定）

## 1. 経緯

design 163で実装した3種類のシェアカード（全制覇/チェックイン/進捗スナップショット）の見た目について、ユーザーから「共有画面のデザインを変えたい」とのフィードバック。第1ラウンド（ミニマル/ダーク＆ゴールド/パスポート風/現状）、第2ラウンド（絵葉書/搭乗券/荷物タグ/古地図/MRTカード/卒業証書）といった「旅行者向け」モチーフを複数提示したが、いずれも「アプリのコンセプトに合わない」と却下。方針転換し、**アプリ自体が既に持つ世界観（スタンプ帳のはんこ表現・卒業アルバム・日記）を延長する**方向で第3ラウンド（スタンプ帳の1ページ／卒業アルバムの1ページ風／日記のワンページ）を提示し、ユーザーが「卒業アルバムの1ページ風」を選択（`前のやつは案2がいいかな`）。

モックファイル（builderはReadツールで参照すること）: `/tmp/claude-1000/-home-masahiko-sg-weekend-app/cf2492da-07b2-4426-a6e8-f71385083029/scratchpad/mock-share-card-concept.html`の「案2」（`.card2`系クラス）。

## 2. 確定デザイン仕様（3種類のカード全てに適用）

共通要素:
- 背景: `var(--warm-white)`（単色、既存design163のグラデーション背景から変更）
- 右上に紙のリボン/しおり風の三角形カットアウト装飾（`clip-path`相当をcanvas上で`ctx.beginPath()`/`ctx.lineTo()`で描画。色は`var(--caramel)`）
- 上部に小さいeyebrow文言（例:「GRADUATION ALBUM PAGE」、レタースペース広め、`var(--warm-gray)`）
- 中央にバッジ/写真画像（円形、box-shadow相当の`ctx.shadowBlur`）
- タイトル（Kaisei Opti太字）
- 手書き風メッセージボックス（`var(--caramel-pale)`背景、角丸、左上に装飾的な引用符「"」、テキストは中央寄せ）
- 下部に達成数値・訪問日等のメタ情報（カード種別ごとに内容を出し分け）

### 2-1. 全制覇カード（`_buildStampCompleteShareCardBlob`）
- タイトル: レベル名
- サブ: 「制覇しました」
- メッセージボックス: 「シンガポールの暮らし、{total}スポットを歩きました。」
- 下部: 「{checked} / {total} スポット達成・在住歴{X年Yか月}」

### 2-2. チェックインカード（`_buildStampCheckinShareCardBlob`）
- 上部にスポット写真がある場合は円形バッジの代わりに角丸の横長写真を使う（モックのpolaroid風処理〈design 121〉と統一感を持たせる。写真が無い場合はバッジ枠のみ、または省略）
- タイトル: スポット名
- サブ: 「訪問しました」
- メッセージボックス: 思い出メモがあれば引用表示、無ければメッセージボックス自体を省略
- 下部: 訪問日

### 2-3. 進捗スナップショットカード（`_buildStampProgressShareCardBlob`）
- タイトル: 「探訪の記録」等の固定文言（特定レベルではなく全体スナップショットのため）
- 上部に在住日数の大きな数字（既存design163のダーク背景ではなく、こちらも`var(--warm-white)`ベースに統一。ただし進捗バー部分は視認性のため`var(--sand)`のトラック色を維持）
- メッセージボックス: 「シンガポール探訪、続けています。」
- 下部: 各レベルの進捗（既存ロジック・未解禁レベルの「？？？」マスキングは無変更のまま維持）

## 3. 変更範囲

`_buildStampCompleteShareCardBlob`/`_buildStampCheckinShareCardBlob`/`_buildStampProgressShareCardBlob`（design 163で実装済み）のcanvas描画ロジック内部のみを変更する。関数シグネチャ・呼び出し元・共有ディスパッチ処理（`_shareStampCardToInstagram`/`_shareStampCardToX`）・エントリーポイント（3箇所のシェアボタン）は無変更。CORS対策・Webフォント読み込み待ちの既存ロジックも無変更のまま維持する。

## 4. スコープ外
共有プレビューミニモーダル（`#stamp-share-preview-modal`）自体のUIデザインは対象外（あくまでカード画像自体のデザイン変更）。`server.js`・データファイルは無変更（`pm2 restart`不要）。
...
# 設計書167 — 探訪の記録バッジを4列グリッドから3列グリッドに変更

（2026-07-26 design 166実装直後、ユーザー「バッジの横並びは三つまでお願いします」「はてなはてなはてなに関しては表示しなくていい、もしくは一段変えて次の行で表示してください」）

## 1. 確定仕様

`public/app.css`の`.stamp-level-badges-grid`（design 166で追加、`grid-template-columns: repeat(4, 1fr);`）を`repeat(3, 1fr);`に変更する。

```css
.stamp-level-badges-grid {
  grid-template-columns: repeat(3, 1fr); /* 4列→3列。4つ目（極めし者）はCSS Gridの標準折り返しで自動的に次の行に表示される */
}
```

CSS Gridの標準的な折り返し挙動により、4つ目のバッジ（極めし者、ロック中は「？？？」表示）は自動的に次の行の左端に配置される（右側2マスは空欄になる）。追加の条件分岐ロジックは不要（ユーザーが提示した2案「非表示」「次の行」のうち、実装コストが低くCSS Gridの自然な挙動で実現できる「次の行」を採用）。

## 2. スコープ外
バッジの内容・タップ挙動（`_renderStampLevelBadges()`等のJSロジック）は変更しない。`public/app.js`・`server.js`・データファイルは無変更（`pm2 restart`不要）。
...
# 設計書169 — filterBatch()のmax_tokensを4000→6000に追加引き上げ

（2026-07-28 design 168実装後の初回cron実行結果、9バッチ中1バッチが依然としてJSON解析エラー〈リトライも失敗〉。4/6→1/9件と大幅改善したが完全解消ではなかったため、ユーザー「少しだけ引き上げしておいて」との指示。`enrichBatch()`側は`max_tokens:6000`で問題報告がないため、同値に揃える）

## 1. 確定仕様

`scripts/filter-events.js`の`filterBatch()`内のHaiku API呼び出しの`max_tokens`を`4000`→`6000`に変更する（`enrichBatch()`と同値に統一）。

## 2. スコープ外
リトライロジック（design 168で追加済み）は変更しない。`enrichBatch()`自体（既に6000）も変更しない。
...
# 設計書172 — シンガポール在住日本人向け生活情報・ニュースのキュレーション表示（行政/天候災害/交通/日本人コミュニティ）

## 0. 前提・調査サマリ

### 0-1. 既存イベント取り込みパイプラインの構成

| ファイル | 役割 |
|---|---|
| `scripts/fetch-events.js` | RSS（`Parser`＝`rss-parser`）・Instagram Graph API（Business Discovery）から記事を取得。`data/source-fetch-state.json`にソースごとの既知GUIDを保存するハイウォーターマーク方式で新着のみ抽出。重複チェック（`deduplicateItems`）後、`filter-events.js`の`filterAndSave()`を呼ぶ |
| `scripts/filter-events.js` | 2段階LLM処理。①`filterBatch()`＝Haiku（`claude-haiku-4-5`、`BATCH_SIZE=10`、`max_tokens:6000`、採否・`type`分類・`who`/`age`/`style`/`genres`/`area`等のメタデータ付与、`scoreThreshold=6`）。②`enrichBatch()`＝Sonnet（`claude-sonnet-4-6`、`ENRICH_BATCH_SIZE=8`）で日本語記事・tips生成。最終的に`data/{city}/events.json`へ追記保存 |
| `scripts/discover-sources.js` / `scripts/analyze-sources.js` | 新規ソース候補の自動発掘（Instagram/RSSをプローブしClaudeでスコアリング）と、既存ソースの採用率に基づく自動`paused`化。週2回（水・日）実行 |
| `data/sources.json` | ソースマスタ（`sg.feeds[]`/`sg.instagramAccounts[]`）。`status: active/paused/rejected`の3値で稼働管理。**2026-08-27時点でInstagramソースは全件`paused`（ユーザー指示で全停止済み）**、RSSフィードのみ稼働中 |
| `data/sg/events.json` | 保存先。`server.js`が`GET /api/events`で都度`fs.readFileSync`して返す（DBなし、メモリキャッシュなし） |
| `scripts/run-fetch-all.sh` | 毎日6:30 SGT。`fetch-events.js --city=sg` → `check-content-integrity.js` → `notify-fetch-summary.js`（LINE通知のみ） |
| `scripts/run-source-analysis.sh` | 水・日7:30 SGT。`discover-sources.js --no-notify` → `analyze-sources.js --no-notify` |

### 0-2. データモデル（`events.json`の`type`）

現行の`type`は`event / show / gourmet / sale / opening`の5種類（ユーザー説明にあった`edu`は現行コードには存在せず、CLAUDE.mdの一部記述が古いままと判明）。主要フィールドは`store`（施設名）・`who`（同行者配列）・`age`（対象年齢配列）・`major_score`（1-5、定番度）・`start_date`/`end_date`・`content`/`content_en`・`tips`/`tips_en`・`area`・`genres`（13種のジャンルタグ）。いずれも「週末に何をして遊ぶか」を軸にした設計で、**行政手続き・災害警報・交通障害・コミュニティ情報とは意味論的に噛み合わない**（`who`＝誰と行くか、`age`＝対象年齢、`major_score`＝定番度、はいずれも生活情報には無意味）。

### 0-3. 表示側（`public/app.js` / `public/index.html`）

- ホーム画面ヘッダーに`#filter-row-category`（カテゴリチップ横スクロール、`data-cat`属性でトグル）。`renderEventCards()`が`EVENT_DATA`（`GET /api/events`のレスポンス）を`filterCats`でフィルタし`CATEGORY_ORDER`順にソート、カード描画は設計書21のDOM差分キャッシュ方式
- ボトムナビは現在**実質3タブ**（ホーム・予定表・設定）。「探訪」（`#nav-course`）は`style="display:none;"`で非表示中（スタンプラリー機能、既存コース機能との統合先。設計書69系）。ユーザーの説明通り、跡地としての転用余地はあるが、現状「探訪」タブの復活自体が別トピックであり、今回のスコープでは「非表示中のタブを新機能に転用する」提案は選択肢として扱う
- `GET /api/weather`エンドポイントは既に実装済み（OpenWeatherMap連携）だが、**`OPENWEATHER_API_KEY`は`.env`にプレースホルダーのまま未設定**（APIキー未設定時は503を返すのみで、天気機能自体は現状使われていない模様）

### 0-4. i18nルール（CLAUDE.md必須）

UI文字列は`STRINGS.ja`/`STRINGS.en`に必ず同時追加、`data-i18n`属性を使う。これは生活情報カテゴリのUI要素（見出し・カテゴリラベル等）にも同様に適用が必須。ただし**記事本文（`content`/`content_en`）自体の生成は既存パイプラインがSonnetで両言語生成する方式を踏襲すれば足りる**。

### 0-5. データ共有の影響（本プロジェクトの必須確認事項）

- `data/sg/events.json`および新設する可能性のある`data/sg/life-info.json`は、いずれも**Web版（テスト環境）とiOS App Store版（本番）が完全に同一のファイルを参照する**。
- `/api/events`のレスポンス構造を変える設計（案Aで後述）を取る場合、**まだアップデートしていない旧バージョンのApp Store版クライアントが、見慣れない`type`値やフィールド欠如（`who`/`age`が空配列等）のデータを描画しようとして予期しない見た目になるリスク**がある。
- 新規エンドポイント（`GET /api/life-info`等）を追加する案（案Bで後述）であれば、**既存の`GET /api/events`のレスポンス構造には一切手を触れないため、旧バージョンAppは影響を受けない**（新エンドポイントを呼ばないだけで、他は今まで通り動く）。
- 詳細な比較・推奨案は「4. データ構造案」「9. データ共有・後方互換性への影響」に記載。
...
1. 行政・ビザ/在留手続き（ICA・MOM・大使館等の制度変更・お知らせ）を見逃したくない
2. 洪水警報・PSI（ヘイズ）・大雨警報など、安全に直結する情報をアプリ内で確認したい
3. MRT遅延・道路規制など、日々の移動に影響する交通情報を知りたい
4. 日本人会・日本人学校・日系スーパーの入荷情報など、日本人コミュニティ発の情報を得たい

という4つのニーズに対し、既存の「週末おでかけイベント」とは別に、生活情報・ニュースをキュレーションして見られるようにしたい。

## 2. 受け入れ基準（案、ユーザー確認前提）

**現時点では「表示場所」「データ構造」ともに複数案があり確定していないため、受け入れ基準は方針確定後に具体化する。以下は暫定的な最大公約数。**

### 正常系（暫定）
- 4カテゴリそれぞれについて、少なくとも1つ以上の情報源から取得された生活情報がアプリ内のどこかで閲覧できる
- 各情報の公開日・出典（ソース名・元記事URL）が明示される
- 日英切り替えで両方の言語で表示が破綻しない（i18n必須ルール準拠）

### 失敗系（暫定）
- 情報源から記事が0件しか取れない期間があっても、アプリがエラー落ちせず「情報がありません」等の空表示になる
- 外部ソース（ICA/MOM/NEA/LTA等）のサイト構造変更・API仕様変更でスクレイピング/フィードが壊れた場合、パイプライン全体ではなく該当ソースのみがサイレントにスキップされる（既存`fetch-events.js`の「フィード取得自体が失敗したソースは状態を更新しない」設計を踏襲）

### エッジケース（暫定）
- 洪水警報・PSI等、**生死や健康に直結しうる情報**が、通常のイベント取得サイクル（現行は毎日6:30 SGT）の頻度では「手遅れ」になりうる。この点は論点4・5で後述する通り未解決
- 同一の行政発表が複数ソース（例: 大使館メルマガ＋MOM公式サイト＋ニュースサイトの転載）から重複して入ってくる場合の重複排除

## 3. スコープ外（明示）

- **プッシュ通知連携（緊急情報の即時配信）**: 既存のWeb Push/APNs基盤は存在するが、生活情報向けの新規配信ロジック構築は今回のスコープ外（論点4で検討はするが実装しない）
- **スクレイピングの新規実装**: ICA/MOM/NEA/LTA等が公式RSS/APIを提供していない場合の非公式スクレイピングコードの実装は行わない（本設計書は調査・方針提示のみ）
- **多言語ソースの機械翻訳精度保証**: 英語の公式発表を日本語要約する際の誤訳リスクへの対策（校閲フロー等）は本設計書のスコープ外
- **ユーザーからの情報投稿・コメント機能**: キュレーション表示のみで、UGC機能は含まない
- **カテゴリごとの通知オンオフ設定UI**: 論点4のプッシュ通知が仮に将来実装されるとしても、今回は設定UIまで作らない
- **BKK/SYD対応**: 現行運用が停止中のため対象外
- **本設計書だけでは実装しない**: この設計書はユーザー承認後、別途detailedな実装設計（データソースの確定、cron頻度の確定、UI配置の確定）を経てからbuilderが着手する前提。**現時点で複数の重要な意思決定（表示場所・データソース・更新頻度）が未確定であるため、本設計書単体では実装フェーズに進めない**

## 4. データ構造の実装方針案

### 案A: 既存`events.json`に新カテゴリとして混在させる（`type: lifeinfo`等）

**方式**: `data/sg/events.json`に`type`の新値（例: `admin`/`weather`/`transport`/`community`、または単一の`lifeinfo`+サブカテゴリフィールド）を追加し、`GET /api/events`のレスポンスにそのまま含める。

**メリット**:
- 既存の取り込みパイプライン（`fetch-events.js`→`filter-events.js`）・カテゴリチップUI（`#filter-row-category`）・カード描画（`renderEventCards()`）をほぼそのまま再利用できる
- 実装コストが低い

**デメリット（重大）**:
- `who`/`age`/`major_score`/`genres`等、生活情報には意味を持たないフィールドを無理やり埋めることになる（Haikuプロンプトが混乱し、既存イベント分類の精度に悪影響を与えるリスク）
- `CATEGORY_ORDER`・カテゴリチップ・`enforceTypeCap()`（カテゴリ別上限管理、`CATEGORY_TARGET_RATIO`）等、既存の「週末おでかけ」を前提にした比率管理ロジックに新カテゴリを混ぜると、既存イベントの表示比率が崩れる可能性がある
- **後方互換性リスクが高い**: `GET /api/events`のレスポンスに新しい`type`値が混入すると、旧バージョンのApp Store版（まだ`admin`等の`type`を知らないクライアント）が、既存の`CATEGORY_ORDER`にない`type`をどう扱うか未検証。既存コード（`CATEGORY_ORDER[a.type] ?? 99`）はフォールバックがあるため描画自体はされるが、フィルタチップに対応するボタンがなければユーザーは「すべて」タブでしか見られず、しかも`who`/`age`が意味不明な値のまま表示されうる
- 安全情報（PSI警報等）を「週末のお出かけ」カードと同じ土俵で表示するのは**情報の重要度に対してUI上の見え方が軽すぎる**（イベントカードに埋もれてしまう）

**総評**: 実装コストは低いが、データの意味論・重要度表現の両面で無理があり、非推奨。

### 案B: 完全に別ファイル・別APIとして分離する（`data/sg/life-info.json` + `GET /api/life-info`）

**方式**: 新規`data/sg/life-info.json`（配列）を新設。フィールドは生活情報に適した最小限のもの（例: `id`/`category`（`admin`/`weather`/`transport`/`community`の4値）/`title`/`title_en`/`summary`/`summary_en`/`source`/`sourceUrl`/`publishedAt`/`severity`（後述）/`fetched_at`）。新規`GET /api/life-info`エンドポイントで返す。取り込みは新規スクリプト（例: `scripts/fetch-life-info.js`）を新設し、既存`fetch-events.js`とは独立したcronジョブとする。

**メリット**:
- 既存`events.json`・`GET /api/events`のレスポンス構造に一切手を加えないため、**旧バージョンApp Store版への影響がゼロ**（後方互換性の懸念が構造的に発生しない）
- カテゴリごとに全く異なる性質（更新頻度・重要度・情報源の形式）を持つデータに対して、それぞれ最適なフィールド・取得頻度・表示ロジックを設計できる
- 将来、天候・災害カテゴリだけ更新頻度を上げる、通知を出す、といった拡張がしやすい（他カテゴリ・既存イベント機能を巻き込まずに済む）

**デメリット**:
- 新規のAPIエンドポイント・データファイル・取得スクリプト・UI（表示場所）を一から作る必要があり、実装コストは案Aより高い
- 既存のカテゴリチップ・カード描画コンポーネントをそのまま流用できないため、新規UIコンポーネントの設計が必要

**総評**: 実装コストは高いが、データの意味論・後方互換性・将来の拡張性の観点で明確に優れる。

### 推奨: 案B（完全分離）

理由:
1. 「安全に直結する情報（PSI・洪水警報等）」を含む以上、既存の「週末お出かけ」向けフィールド（`who`/`age`/`major_score`）に無理やり当てはめること自体がデータ設計として不適切
2. CLAUDE.mdに明記された「データ構造の破壊的変更はApp Store版のリリースと同時に行う」「APIレスポンスの構造を非互換に変更しない」という既存ルールに照らすと、既存`events.json`/`GET /api/events`に手を入れる案Aは慎重な検討・段階的リリースが必須になり、リスクとコストに見合わない
3. 4カテゴリはそれぞれ更新頻度・緊急度が大きく異なる（後述の論点4・5参照）。将来的にカテゴリごとに異なる扱い（例: 天候だけ高頻度更新、コミュニティは低頻度）をする際、独立したデータ構造の方が柔軟

## 5. 表示場所の実装方針案

現在ボトムナビの「探訪」（`#nav-course`）は`display:none`で非表示中（スタンプラリー機能の入口）。この扱いをどうするかは今回のスコープに含めるかユーザー判断が必要なため、複数案を提示する。

### 案1: ホーム画面に新カテゴリチップとして追加する

`#filter-row-category`に「生活情報」チップを追加し、タップすると生活情報のみのリストに切り替わる。

- メリット: 導線がシンプル、既存のホーム画面を訪れる動線に自然に乗る
- デメリット: 案Bのデータ構造（別API）と組み合わせる場合、`renderEventCards()`の`EVENT_DATA`ベースの描画ロジックとは別のレンダリング分岐が必要になり、実装がやや複雑になる。また、イベント（お出かけ情報）と生活情報という性質の異なる情報が同じ画面・同じUIパターンに混在することで、ユーザーが「イベント探し」の最中に無関係な行政情報が挟まる体験になりうる

### 案2: ホーム画面内に独立したセクション（カード一覧の上または専用タブ内タブ）を新設する

ホーム画面のカテゴリチップ行とは別に、画面上部に「お知らせ」的な横スクロールカルーセルや、常時折りたたみ表示のセクションを新設する。

- メリット: 既存のイベント一覧のフィルタリング・ソートロジックを一切変更せずに追加できる。安全情報等は「常に目に入る」位置に置ける
- デメリット: ホーム画面が縦に長くなる・情報過多になるリスク。4カテゴリ全部をこの方式で表示すると、通常のイベント探しの妨げになる可能性がある

### 案3: 非表示中の「探訪」タブ跡地、または新規5つ目のボトムナビタブとして独立させる

「探訪」タブは現状スタンプラリー機能の入口として予約されており、単純に生活情報の入れ物として転用すると設計書69系の資産と競合する。新規5つ目のタブ（例: 「くらし」）を追加する案もありうる。

- メリット: 4カテゴリを落ち着いて閲覧できる専用の場所ができる。イベント探しの体験を一切汚さない
- デメリット: ボトムナビのタブ数が増える（現行UIは4タブ設計、5つ目を追加するとUI全体のバランス調整が必要）。「探訪」タブの今後の扱い（スタンプラリー機能を今後も非表示のままにするのか、いずれ復活させるのか）という別の意思決定が絡む。新規タブ追加は明確にスコープ拡大であり、**ユーザーの明示的な承認が必要**

### 推奨: 案2（ホーム画面内の独立セクション）をベースに、緊急度の高いカテゴリ（天候・災害）のみ強調表示する複合案

理由:
- 新規タブ追加（案3）はUI構成へのインパクトが大きく、「今回のスコープに含めるかはユーザー判断が必要」との前提が示されている通り、本設計書だけで断定すべきでない
- 案1（チップ追加）は性質の異なる情報を同じ土俵に混在させることになり、案Aと同様の懸念がある
- 案2は既存のホーム画面構造を汚さずに追加でき、かつ「安全情報だけは目立たせる」という重要度に応じた出し分けができる

ただし、**この推奨は暫定であり、実際のUI配置（セクションの高さ・折りたたみの有無・4カテゴリを同一セクション内でどうタブ分けするか等）はモックアップベースでユーザーに複数案を見せて選んでもらうプロセスが必要**。本設計書の段階では確定させない。

## 6. カテゴリ別ソース選定案（要ユーザー確認）

Web検索ができないツール制約のため、既存コードのAPIキー設定（`.env`）から使えそうな外部連携の有無を確認した範囲でのみ記載する。**具体的なRSS URL・API仕様の実在性・利用規約の確認はいずれも未実施であり、すべて「要ユーザー確認」として扱う。**

### 6-1. 行政・ビザ/在留手続き（ICA・MOM・大使館）
- ICA（Immigration & Checkpoints Authority）・MOM（Ministry of Manpower）が公式RSSフィードを提供しているかは**不明（要ユーザー確認）**。一般的に政府系サイトはRSS非提供のケースが多く、非公式スクレイピングが必要になる可能性がある
- 在シンガポール日本国大使館の「お知らせ」ページ・メールマガジンの形式も**不明（要ユーザー確認）**
- 既存の`.env`にはICA/MOM関連のAPIキー・連携設定は一切存在しない
- 代替案: 既存のRSS取得基盤（`rss-parser`）がそのまま使えるフィードが見つかれば流用可能。見つからない場合はスクレイピングの実装要否を別途検討する必要がある（本設計書のスコープ外、論点として明記のみ）

### 6-2. 天候・災害・ヘイズ（PSI）
- NEA（National Environment Agency）が公式API（myENV等）を提供している可能性があるが、既存`.env`にNEA関連のAPIキーは存在しない
- **既存インフラとして`GET /api/weather`（OpenWeatherMap連携）が既に実装済みだが、`OPENWEATHER_API_KEY`はプレースホルダーのまま未設定で機能していない**。天候情報自体はOpenWeatherMapで一般的な気象データは取れる可能性があるが、**PSI指数・ヘイズ情報・シンガポール政府が出す洪水警報・大雨警報はOpenWeatherMapの守備範囲外**であり、NEA等シンガポール政府系の専用データソースが別途必要
- 代替案: NEAの一般公開データAPI（`api.data.gov.sg`等）の存在は本プロジェクトのコードベース上には確認できず、**要ユーザー確認**
- **重要度が最も高いカテゴリだが、確実なデータソースが現時点で1つも確定していない**

### 6-3. 交通・MRT運行情報
- LTA（Land Transport Authority）の`DataMall`APIが一般的に知られているが、既存`.env`にLTA関連のAPIキーは存在しない
- SBS Transit/SMRTのTwitter（X）アカウントで運行情報を発信している可能性がある。本プロジェクトには`X_BEARER_TOKEN`等X API認証情報が`.env`に存在するが、**現在X APIはクレジット枯渇により停止中**（設計書53、`scripts/post-to-x.js`は`--to-line`モードに切り替え済み、読み取り用途にこのトークンを転用できるかは別途確認が必要）
- 代替案: LTA DataMall APIの新規登録・APIキー取得が必要になる可能性が高い（**要ユーザー確認・要ユーザー作業**）

### 6-4. 日本人コミュニティ情報
- `data/sources.json`に**既存で登録済みのRSSソースの中に、日本人コミュニティ向けと思われるものが2件存在する**: `AsiaX`（`https://www.asiax.biz/feed/`）・`SingaWeb`（`https://www.singaweb.info/feed`）。いずれも`status: active`。これらが実際に日本人コミュニティ・生活情報寄りのメディアかどうかは記事内容の確認が必要（**要ユーザー確認**、URLパターンからシンガポール在住日本人向けメディアである可能性が高いと推測されるのみ）
- ユーザー言及の「SavvyJapanSG」については、本プロジェクトのコードベース上に一切の記録・設定がなく、RSS配信の有無も含めて**完全に要ユーザー確認**
- 日本人会・日本人学校の公式サイトがRSSを提供しているかも不明（**要ユーザー確認**）
- 日系スーパー（明治屋・OG（Overseas Grocery）等）の入荷情報はSNS（Instagram等）での発信が主体である可能性が高いが、**現在Instagramソースは全件`paused`（2026-08-27にユーザー指示で全停止済み）**。日本人コミュニティ情報のためだけにInstagram取得を部分的に再開するかは要検討・要ユーザー判断

### 6-5. ソース選定に関する結論
4カテゴリのうち、**確実に実装できる見込みがあるのは「6-4 日本人コミュニティ情報」の一部（既存RSSソース2件の内容確認＋活用）のみ**。他3カテゴリ（行政・天候災害・交通）は、いずれも専門の政府系データソースへの新規連携が必要で、その存在・仕様・利用規約が本設計書の調査範囲内では確認できなかった。**この設計書だけでは4カテゴリすべてを同時に実装着手できる状態ではなく、カテゴリごとにソース確定の追加調査（可能であればユーザーによるWeb検索、またはこの後の別セッションでのWeb検索可能なツール構成での調査）が必要**。

## 7. フィルタリング方式の検討

### 7-1. 既存方式（Haiku一次選別→Sonnet記事化）がそのまま使えるか

- **行政・コミュニティ情報**: 概ね流用可能。既存の「採否判定→日本語記事生成」の2段階構成は、行政発表の要約や日本人コミュニティニュースの紹介文生成にも応用できる
- **天候・災害情報**: 既存方式は**不向き**。PSI指数・洪水警報は「数値・現在の状態」であり、LLMによる記事生成・要約というより「構造化データをそのまま表示する」方が適切（誤読・幻覚のリスクを避けるため、数値情報はLLMを介さずAPIレスポンスを直接表示する設計が望ましい）
- **交通・MRT運行情報**: 同様に「現在遅延しているか否か」という状態情報が中心であれば、LLM処理よりAPIの構造化データを直接表示する方が適切と考えられる

### 7-2. 即時性・重要度の要求

- 既存パイプラインは**毎日6:30 SGT 1回のみ**（cron `run-fetch-all.sh`）。洪水警報・PSI急上昇のような「今起きていること」を伝える情報にとって、1日1回の更新頻度は明らかに不十分
- 緊急度の高い天候・災害情報については、**別経路（高頻度ポーリング、または該当APIがWebhook/プッシュを提供するならそちらを利用）が必要**。既存のプッシュ通知基盤（Web Push/APNs、`sendPushToAll()`等）を転用できる可能性はあるが、これは論点4の追加検討事項でありスコープ外
- 行政手続き・コミュニティ情報は、既存の1日1回程度の更新頻度でも実用上大きな問題はないと考えられる

### 7-3. 結論
4カテゴリを一律の方式・頻度で扱うのは不適切。**「行政・コミュニティ」＝既存パイプライン踏襲（LLM処理・低頻度）と、「天候・災害・交通」＝構造化データ直接表示・高頻度ポーリング（LLM処理は最小限または無し）、の2グループに分けて設計する方が実態に即している**。ただし後者の具体的な実装（ポーリング頻度、プッシュ通知連携の要否）は未確定。

## 8. 既存機能への影響の検討

### 8-1. `data/sources.json`の`status`管理への影響
- 案B（別ファイル・別スクリプト）を採用する場合、生活情報用のソースマスタは`data/sources.json`に相乗りさせず、**別ファイル（例: `data/life-info-sources.json`）として独立させることを推奨**。理由: 既存の`sources.json`は`sg.feeds[]`/`sg.instagramAccounts[]`という「イベント情報源」を前提にした構造であり、`discover-sources.js`/`analyze-sources.js`の自動候補探索・自動停止ロジックも「採用率（イベントとして採用されたか）」を前提に組まれている。生活情報の採否基準はイベントとは全く異なるため、同じ自動化ロジックを流用するのは不適切
- 既存の`analyze-sources.js`による自動`paused`化ロジックを生活情報ソースにそのまま適用すると、たとえば「MOM公式サイトからの情報は毎回ほぼ全件採用されるべき」ような性質のソースが、既存の「採用率が低いと自動停止」という前提と噛み合わない可能性がある

### 8-2. cron頻度への影響
- 現行の水・日7:30 SGT（週2回）の`run-source-analysis.sh`は新規ソース候補探索用であり、生活情報の「鮮度」とは無関係。生活情報の取得自体は新規cronジョブとして独立させる必要がある（既存の`run-fetch-all.sh`に相乗りさせない）
- 天候・災害カテゴリについては、毎日1回はおろか週2回でも全く不十分（論点7-2参照）。**具体的な頻度（例: 1時間ごと等）はデータソースが確定していない現状では決定できない**

### 8-3. i18n対応への影響
- 生活情報は日本語ネイティブソース（大使館・日本人コミュニティメディア等）が多いと想定されるが、行政・交通情報は英語ソース（ICA/MOM/LTA/NEA等の公式発表）が中心になる可能性が高い
- 既存パイプラインと同様、**Sonnetによる日英両言語での記事生成が必要**（CLAUDE.mdのi18n必須ルールに準拠）。ただし天候・災害のような構造化データ（PSI指数等）は、そもそも言語依存する自由文が少ないため、ラベル文言のみ`STRINGS.ja`/`STRINGS.en`で対応すれば足りる可能性がある

## 9. データ共有・後方互換性への影響（必須確認事項）

1. **後方互換性**: 案B（完全分離、`data/sg/life-info.json` + 新規`GET /api/life-info`）を採用する限り、既存`GET /api/events`のレスポンス構造・`events.json`のスキーマには一切変更を加えないため、**旧バージョンのApp Store版アプリは新機能の存在を認識しないだけで、既存動作は一切壊れない**。もし案A（既存`events.json`への混在）を選んだ場合は、新しい`type`値・想定外のフィールド値を旧バージョンのクライアントがどう描画するか実機検証が必須になる（本設計書では推奨しないため詳細検証は行っていない）
2. **影響範囲**: 案Bであれば、この変更は「新しいAPIエンドポイント・新しい画面/セクションの追加」のみであり、**既存のWeb版・iOS App Store版のいずれにも、新機能をリリースするまでは何の影響も与えない**。新機能公開後は当然Web版・iOS版の両方に同時に反映される（データ層・APIはサーバー1つで共有のため、フロントエンドのコードが対応していなければ単に「呼ばれない・使われない」だけ）
3. **リリースタイミング**: 案Bの新規エンドポイント追加自体はAPIの後方互換性を壊さないため、**API変更とアプリ（iOS）リリースを厳密に同時にする必要はない**。ただし、新機能のUI（フロントエンド）はWeb版・iOS版で完全に同じコード（Capacitorのローカルバンドル方式）のため、**フロントエンドのコード変更は必然的にWeb版に即座に反映され、iOS版には次回TestFlightビルド以降に反映される**、という既存の運用ルール通りの段階差は生じる。緊急性の高い天候・災害情報を扱うカテゴリについては、iOS版へのリリースがWeb版より遅れる期間中、iOS版ユーザーだけがこの新機能を使えない状態が一定期間続く点に留意が必要（アプリストア審査に依存するため、意図的に足並みを揃えることは難しい）

## 10. 変更するファイル一覧（案B採用時の想定、確定ではない）

**注意: 以下は「もし案Bで進める場合」の想定リストであり、表示場所・ソース確定前の暫定案。確定後に別途詳細設計が必要。**

- 新規: `data/sg/life-info.json`（データ本体、gitignore対象）
- 新規: `data/life-info-sources.json`（ソースマスタ、行政・コミュニティ用のRSS/フィード管理。天候・交通はAPIポーリング前提のため別管理になる可能性）
- 新規: `scripts/fetch-life-info.js`（行政・コミュニティ向け、既存`fetch-events.js`/`filter-events.js`のパターンを踏襲した取得・LLM処理スクリプト）
- 新規: `scripts/fetch-weather-transport-alerts.js`（仮称、天候・交通向け。構造化データの直接ポーリング、LLM処理は最小限）
- 変更: `server.js`（新規`GET /api/life-info`エンドポイント追加のみ、既存エンドポイントは無変更）
- 変更: `public/index.html`（新規UIセクション/チップ/タブの追加。表示場所は論点5で未確定）
- 変更: `public/app.js`（新規描画関数、`STRINGS.ja`/`STRINGS.en`への新規i18nキー追加）
- 変更: `public/app.css`（新規UIのスタイル）
- 新規（必要なら）: crontab新規エントリ（頻度未確定）

## 11. データモデルの変更（暫定案）

```json
// data/sg/life-info.json（案）
[
  {
    "id": "li_xxx",
    "category": "admin",       // admin | weather | transport | community
    "severity": "normal",       // normal | warning | critical（天候・交通向け、行政・コミュニティは常にnormal想定）
    "title": "MOM、就労ビザ〇〇制度を変更",
    "title_en": "MOM updates ... visa rules",
    "summary": "...",
    "summary_en": "...",
    "source": "MOM official",
    "sourceUrl": "https://...",
...
**未確定事項**: `severity`フィールドの必要性・値の粒度、天候カテゴリでの数値データ（PSI指数そのもの等）の持たせ方、交通カテゴリでの路線別・駅別の構造化方法は、ソース確定後でないと具体化できない。

## 12. APIの変更（暫定案）

- 新規: `GET /api/life-info?city=sg&category=admin|weather|transport|community`（既存`GET /api/events`と同様、認証不要・`fs.readFileSync`都度読み込み）
- 既存`GET /api/events`・`GET /api/sales`・その他既存APIへの変更は無し（案B前提）

## 13. フロントエンドの変更（暫定・未確定）

表示場所（論点5）が確定していないため、具体的なマークアップ・関数設計は本設計書では行わない。確定後、以下を含む詳細設計が必要:
- 新規セクション/タブ/チップのHTML構造
- 新規カテゴリ用のカードデザイン（既存`.spot-card`をそのまま使うか、生活情報専用の軽量デザインにするか）
- 天候・災害カテゴリの「重要度が高い情報を目立たせる」ための視覚的な差別化（バッジ・色分け等）
- i18nキー追加（見出しラベル、カテゴリ名等）

## 14. リスク・未解決の質問（ユーザーへの確認が必須）

1. **表示場所をどうするか**: 案1（既存チップ追加）/ 案2（ホーム画面内独立セクション）/ 案3（新規タブ or 探訪タブ跡地）のいずれを望むか。新規タブ追加はスコープ拡大になるため明示的な承認が必要
2. **4カテゴリを同時に着手するか、優先順位をつけるか**: ソース確度に大きな差があり（コミュニティ情報のみ既存RSS候補あり、他3カテゴリは未確定）、全カテゴリを同時にリリースしようとすると着手できないカテゴリに引きずられる。カテゴリ別に段階リリースする案も検討の余地がある
3. **行政（ICA/MOM/大使館）・天候（NEA/PSI）・交通（LTA）の具体的なデータソース確定**: 本設計書の調査範囲（Web検索不可）では確定できなかった。ユーザー側での確認、または次回Web検索可能な環境での追加調査が必要
4. **天候・災害情報の即時性要求にどう応えるか**: 既存の1日1回cronでは実用に耐えない可能性が高い。高頻度ポーリング・プッシュ通知連携（新規実装が必要）のいずれを採用するか、またはこの機能を「参考情報」程度の位置づけに留め即時性を求めないか、方針決定が必要
5. **SavvyJapanSGを含む日本人コミュニティメディアのRSS/API提供有無**: 完全に未確認。既存の`AsiaX`/`SingaWeb`（RSS登録済み）が実際に該当ジャンルの記事を含むかも確認が必要
6. **`OPENWEATHER_API_KEY`を実際に取得・設定するか**: 天候カテゴリの一部（一般気象情報）に使える可能性があるが、PSI・洪水警報等シンガポール政府系の専用情報は別途ソースが必要なため、OpenWeatherMap単体では要件を満たせない
7. **ソースの信頼性・法的リスク**: 政府機関の発表を要約・転載する形になるため、著作権・利用規約上の扱い（原文リンクの明示で足りるか等）を確認する必要があるかもしれない（本プロジェクトの既存イベント取り込みと同様の扱いで問題ないか、ユーザー・法務判断が必要な可能性）

---

## 15. 承認状況

未承認。上記「14. リスク・未解決の質問」7点について、特に(1)表示場所、(2)カテゴリの優先順位・段階リリースの要否、(3)行政/天候/交通の具体的データソースの3点はユーザーの意思決定・追加調査なしには実装設計に進めない。ユーザー承認・回答待ち。

## 16. 設計書172 追記（2026-08-28、ユーザーとの調査を経て方針確定）

上記14節の未解決事項のうち、以下がユーザーとの対話・実RSS検証を経て確定した。

### 16-1. 表示場所（確定）
既存イベントとは完全に分離。**ボトムナビに新規5つ目のタブ「ニュース」（📰）を新設**（案3を採用）。非表示中の「探訪」タブ（スタンプラリー機能）とは無関係の別枠として扱い、`#nav-course`の扱いには一切手を加えない。

### 16-2. データソース（確定、実際にcurlで疎通確認済み）

| ソース | URL | 状態 | 備考 |
|---|---|---|---|
| CNA (Singapore) | `https://www.channelnewsasia.com/api/v1/rss-outbound-feed?_format=xml&category=10416` | ✅疎通確認済み | 総合ニュース。交通(LTA)・労働規則(MOM系)等も混在。**主軸ソース** |
| Mothership.sg | `https://mothership.sg/feed/` | ✅疎通確認済み | 総合ニュース。行政系トピックも含む。CNAの補完 |
| JCCI（シンガポール日本商工会議所） | `https://www.jcci.org.sg/feed/` | ✅疎通確認済み | セミナー・月報中心。ビジネス色が強いが日本人コミュニティ関連情報として採用 |

**不採用と判断したソース**:
- 在シンガポール日本国大使館: RSS URLへのアクセスが403（WAF）、取得不可
- SingaLife / SingaWeb: RSSは有効だが記事内容が美容・グルメ等の広告記事中心で「ニュース」に該当しない
- Straits Times: RSSは応答するが記事タイトルが古いカテゴリ名の羅列で内容の鮮度・信頼性が確認できず不採用
- TODAY online: レスポンス内容が破損しており（bot対策の可能性）不採用

**日本人コミュニティ情報カテゴリの限界（既知の妥協点）**: 大使館・日本人会等のピンポイントなソースは見つからず、JCCIで一部カバーする形になる。将来的に良いソースが見つかれば追加する前提とし、v1はこの3ソースで開始する。

### 16-3. データモデル（確定、`severity`フィールドは不採用に変更）

```json
// data/sg/life-info.json
[
  {
    "id": "li_xxx",
    "category": "admin",       // admin | weather | transport | community
    ...
    "source": "CNA",
    "sourceUrl": "https://...",
    "publishedAt": "2026-08-28T18:46:34+08:00",
    "fetched_at": "2026-08-28"
  }
]
```
`severity`（重要度）フィールドは、今回のソースが構造化された警報データではなく報道記事ベースであるため見送る（設計書172 §11時点の暫定案から変更）。

### 16-4. フィルタリング方式（確定）
新規`scripts/fetch-life-info.js`を新設。既存`fetch-events.js`のRSS取得パターン（`rss-parser`、ハイウォーターマーク方式で`data/source-fetch-state.json`相当の仕組みを流用または新規`data/life-info-fetch-state.json`を新設）を踏襲。
- Haiku一次選別: 「シンガポール在住日本人の生活に関係あるか」（行政手続き・天候/災害・交通・近隣コミュニティ制度等）を判定し、`category`（admin/weather/transport/community）を付与。スポーツ・芸能・事件事故等の無関係記事は却下
- Sonnet記事化: 採用された記事を日本語要約＋英語要約の両方で生成（既存`enrichBatch()`パターン踏襲）

### 16-5. 更新頻度（確定）
新規cronジョブとして独立（既存`run-fetch-all.sh`には相乗りしない）。**1日1回**からスタート（既存イベント取得と同様の頻度感、緊急速報的な即時性は今回のスコープでは求めない）。実運用を見て頻度調整の余地あり。

### 16-6. API・UI（確定方針、詳細はbuilder実装時に確定）
- 新規`GET /api/life-info?city=sg&category=admin|weather|transport|community`（`city`パラメータは将来のBKK/SYD対応を見据えて残すが、v1は`sg`のみデータが存在）
- 新規画面（ボトムナビ「ニュース」タップで表示）。カテゴリチップで絞り込み、カードタップで元記事URLをブラウザで開く（既存`Browser.open()`/`window.open()`パターン踏襲）
- i18n: `navNews`（ja「ニュース」/en「News」）を含む新規キーをja/en同時追加（CLAUDE.md必須ルール準拠）

### 16-7. 承認状況
2026-08-28 ユーザーが上記方針で実装を承認（「進めます」）。**設計書172は実装フェーズに移行**。

### 16-8. ホーム画面プレビュー追加（2026-08-28追記）
ユーザー要望「アプリを開くとパッとシンガポールの情報が見れるようにしてほしい」を受け、16-6のニュースタブ（フル一覧）に加えて、**ホーム画面最上部（ヘッダー直下、`#filter-row-category`の手前）に生活情報のプレビューセクションを新設**する。
- 直近2〜3件（カテゴリ横断、公開日新しい順）を横スクロールカード or 縦積みの軽量カードで表示
- カードタップで元記事URLを直接開く（ニュースタブへの遷移を挟まない、ワンタップで完結）
- 「もっと見る」リンクでニュースタブ（フル一覧）へ遷移
- `GET /api/life-info`を流用（`limit`パラメータ等で件数制御、または全件取得しフロントで先頭数件を使う）
- 生活情報が0件の期間はプレビューセクション自体を非表示にする（既存の「データがなければ表示しない」方針を踏襲）
